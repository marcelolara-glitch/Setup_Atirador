...
"""
=============================================================================
SETUP ATIRADOR v6.6.0 - Scanner Profissional de Criptomoedas
=============================================================================
...
Autor: Manus AI | v4.1→v6.6.0 (revisão Claude/Anthropic)
=============================================================================
...
def _tg_call_long(r: dict, ctx: dict, pode_operar: bool, state: dict = None) -> str:
    ...
    msg = (
        ...
        f"   {"⚠️ MARGEM ACIMA DO LIMITE — gerencie manualmente" if t.get('margem_excedida') else "Margem dentro do limite"}\n"
...
def _fetch_okx_tickers_with_oi():
    ...
    try:
        ...
        # Requisição 3: Funding Rates reais [v6.6.0]
...
def _fetch_okx_funding_rates(symbols_okx: list) -> dict:
    """
    [v6.6.0] Busca Funding Rates reais da OKX via endpoint dedicado.
    ...
    headers = {"Accept-Encoding": "gzip, deflate",
               "User-Agent": "Mozilla/5.0 (compatible; scanner/6.6.0)"}
    n_ok = 0; n_err = 0
...
def fetch_perpetuals():
    ...
    LOG.info("📡 [v6.6.0] Iniciando busca de tickers — hierarquia OKX → Gate.io → Bitget")
...
def score_bollinger(d, direction="LONG"):
    ...
    # [v6.6.0] Lógica de descarte assimétrica por direção:
...
def calc_trade_params(price, atr, score=0, threshold=0):
    ...
    Garantia: margem_por_trade ≤ MARGEM_MAX_POR_TRADE em qualquer cenário.
    [v6.6.0] threshold: aviso de margem só é logado se score >= threshold.
    """
...
async def run_scan_async():
    ...
    LOG.info("🚀 Setup Atirador v6.6.0 | Arquitetura 3 Camadas (LONG+SHORT) | Iniciando scan...")
    t_start = time.time()
    ...
    async with aiohttp.ClientSession() as session:
        ...
        # [v6.6.0] Se o batch de candles falhou completamente (retornou {}),
        # tenta requests individuais por coluna para isolar qual nome está quebrando.
        if not tv_15m_candles:
            LOG.warning("  ⚠️  [v6.6.0] Batch candles 15m falhou — tentando colunas individualmente para diagnóstico")
            cols_ok = []
            cols_fail = []
            for col in COLS_15M_CANDLES:
                ...
                if test_result:
                    ...
                    LOG.error(f"  ❌  [v6.6.0] Coluna inválida detectada: {col}")
            if cols_ok:
                LOG.info(f"  🔄  [v6.6.0] Rebuscando candles com {len(cols_ok)} colunas válidas: {cols_ok}")
                tv_15m_candles = await fetch_tv_batch_async(session, symbols_15m, cols_ok)
            if cols_fail:
                LOG.warning(f"  ⚠️  [v6.6.0] Colunas removidas por falha de API: {cols_fail}")
        ...
        report  = f"{'='*58}\n"
        report += f"🎯 SETUP ATIRADOR v6.6.0\n"
        report += f"📅 {ts_full}\n"
        ...
        report += f"💼 GESTÃO DE RISCO — Sizing Risk-First [v6.6.0]\n"
        ...
        LOG.info(report)
        LOG.info(f"✅ Scan v6.6.0 concluído em {elapsed:.1f}s | Fonte: {DATA_SOURCE} | "
