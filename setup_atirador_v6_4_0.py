...
"""
=============================================================================
SETUP ATIRADOR v6.4.0 - Scanner Profissional de Criptomoedas
=============================================================================
...
v6.1.2 — Notificações Telegram (webhook) com dois modos de mensagem:

  [v6.1.2 — TELEGRAM WEBHOOK]
    Dois tipos de mensagem enviados automaticamente ao final de cada scan:

    TIPO 1 — ALERTA DE CALL (score ≥ threshold):
      Enviado somente quando há setup com score suficiente para operar.
      Conteúdo completo: direção, score, entrada, SL, TP1/2/3, alavancagem,
      razões técnicas principais e contexto de mercado.
      Formatação com emojis e negrito para leitura rápida no celular.

    TIPO 2 — HEARTBEAT (toda rodada, independente de alerts):
      Mensagem compacta enviada a CADA execução mesmo sem alertas.
      Objetivo: verificar consistência do sistema, detectar falhas silenciosas
      e acompanhar a evolução dos scores sem precisar abrir arquivos de log.
      Conteúdo: timestamp, FGI, BTC 4H, universo, gate counts LONG/SHORT,
      score máximo LONG, score máximo SHORT, fonte de dados, tempo de execução.
      Se o heartbeat para de chegar → script parou ou travou.

    Configuração: variáveis TELEGRAM_TOKEN e TELEGRAM_CHAT_ID no bloco
    de configuração. Deixar vazias para desativar sem alterar código.

    Falha de envio: silenciosa (WARNING no log, scan continua normalmente).
    O Telegram nunca bloqueia a execução do scanner.

v6.4.0: A1 sizing risk-first, A2 recalibrar score/thresholds/tabela alav,
        A9 data_quality separado do setup_score
v6.3.0: A3 candles bearish 15m, A4 candle lock, A6 oi_estimado flag, A8 obs_short
v6.4.0: A1 sizing risk-first, A2 recalibrar score/thresholds/tabela alav,
        A9 data_quality separado do setup_score
        (A5 venue única: pendente decisão arquitetural do usuário)
v6.2.0: KLINE_TOP_N→20 (LONG+SHORT), SR_PROXIMITY→2.5%, RSI<30 descarta SHORT no Gate 4H,
        P9 OI crescente no score, threshold SHORT espelha LONG, score histórico por token no
        estado diário (48 rodadas/token, limpeza automática), heartbeat Telegram expandido
        com radar top 3 e evolução de score, KLINE_CACHE_TTL_H→1h
v6.1.1: Fixes: os.getenv, dead code, heartbeat duplicatas
v6.0: SHORT bidirecional, pilares bearish espelhados, exclusividade LONG/SHORT
...
Autor: Manus AI | v4.1→v6.4.0 (revisão Claude/Anthropic)
=============================================================================
...
def _tg_call_long(r: dict, ctx: dict, pode_operar: bool, state: dict = None) -> str:
    """
    [v6.2.0] Mensagem de ALERTA LONG — dados completos para execução imediata.
    Inclui OI, evolução de score e todas as razões técnicas.
    """
    t      = r["trade"]
    score  = r["score"]
    sym    = r["base_coin"]
    ...
    razoes = "\n💡 ".join(r.get("reasons", [])[:5])  # até 5 razões

    oi_usd = r.get("oi_usd", 0)
    oi_str = f"${oi_usd/1e6:.1f}M" if oi_usd else "N/D"

    msg = (
        f"🚀 <b>LONG {sym}</b>{bloq} {trend}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📊 Score: <b>{score}/26</b> (mín≥{thr}) | Evolução: {trend}\n"
        f"📈 4H: {r.get('summary_4h','?')} | 1H: {r.get('summary_1h','?')}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"💡 {razoes}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        ...
        f"🛑 SL:  ${t['sl']:.4f}  (-{t['sl_distance_pct']:.2f}%) ← abaixo\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"⚡ Alavancagem: <b>{t['alavancagem']}x</b>\n"
        f"   Notional: ${t.get('notional_usd',0):.0f} | Margem alocada: ${t.get('margem_usd',0):.2f}\n"
        f"   Risco: ${t['risco_usd']:.2f} | Ganho esperado (RR1:2): ${t['ganho_rr2_usd']:.2f}\n"
        f"📊 Vol 24h: ${r['turnover_24h']/1e6:.1f}M | OI: {oi_str}\n"
        f"   Funding Rate: {r.get('funding_rate',0):.4%}\n"
        f"⏰ Gestão: mover SL para breakeven +0.5% após TP1"
    )
    return msg
...
def _tg_call_short(r: dict, ctx: dict, pode_operar: bool, state: dict = None) -> str:
    """
    [v6.2.0] Mensagem de ALERTA SHORT — dados completos para execução imediata.
    """
    t      = r["trade_short"]
    ...
    razoes = "\n💡 ".join(r.get("reasons_short", [])[:5])

    oi_usd = r.get("oi_usd", 0)
    oi_str = f"${oi_usd/1e6:.1f}M" if oi_usd else "N/D"

    msg = (
        f"📉 <b>SHORT {sym}</b>{bloq} {trend}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📊 Score: <b>{score}/26</b> (mín≥{thr}) | Evolução: {trend}\n"
        f"📉 4H: {r.get('summary_4h','?')} | 1H: {r.get('summary_1h','?')}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
...
def _tg_relatorio_rodada(ctx: dict, total_items: int, qualificados: int,
                          ...
                          state: dict, tokens_sem_dados: list) -> str:
    """
    [v6.2.0] Relatório completo de rodada — substitui o heartbeat compacto.

    Enviado a cada execução, independente de haver calls.
    Inclui: contexto de mercado, pipeline, radar top-7 LONG e SHORT com
    evolução de score (↑↑ ↑ → ↓ ↓↓ 🆕) e tendência de OI.
    Limite: ~4096 chars (Telegram). Estrutura prioriza o que importa primeiro.
    """
    ts      = datetime.now(BRT).strftime("%d/%m/%Y %H:%M BRT")
    fgi_val = ctx["fg"]
    fgi_ico = "🔴" if fgi_val <= 20 else ("🟠" if fgi_val <= 35 else ("🟡" if fgi_val <= 55 else ("🟢" if fgi_val >= 70 else "⚪")))
    fgi_txt = "Medo Extremo" if fgi_val <= 20 else ("Medo" if fgi_val <= 40 else ("Neutro" if fgi_val <= 60 else ("Ganância" if fgi_val <= 80 else "Ganância Extrema")))
    ...
    alertas_short = [r for r in results_short if r.get("score_short",0)  >= ctx["threshold_short"]]

    # ── Linha de calls ────────────────────────────────────────────────────────
    if alertas_long or alertas_short:
        call_parts = []
        for r in alertas_long:  call_parts.append(f"🚀{r['base_coin']}({r['score']})")
        for r in alertas_short: call_parts.append(f"📉{r['base_coin']}({r.get('score_short',0)})")
        calls_line = f"🔥 <b>CALLS ATIVOS: {' | '.join(call_parts)}</b>"
    else:
        max_l = max((r["score"] for r in results), default=0)
        max_s = max((r.get("score_short",0) for r in results_short), default=0)
        calls_line = f"💤 Sem calls — máx LONG {max_l}/28 | máx SHORT {max_s}/28"

    # ── Radar LONG (até 7 tokens) ──────────────────────────────────────────
    radar_long_lines = []
    for r in results[:7]:
        trend = get_score_trend(state, r.get("symbol",""), "LONG")
        score = r["score"]
        s4h   = r.get("summary_4h","?")
        sym   = r.get("base_coin","?")
        oi_sc = r.get("oi_score", 0)
        oi_tag = f" OI{'+' if oi_sc>0 else ''}{oi_sc}" if oi_sc != 0 else ""
        fr    = r.get("funding_rate", 0)
        fr_tag = f" FR{fr:.3%}" if fr != 0 else ""
        dq    = r.get("data_quality", 1.0)
        dq_tag = f" ⚠️DQ{dq:.0%}" if dq < 1.0 else ""   # [v6.4.0 A9]
        radar_long_lines.append(f"  🟢 <b>{sym}</b> {score}/28 {trend} | {s4h}{oi_tag}{fr_tag}{dq_tag}")

    # ── Radar SHORT (até 7 tokens) ─────────────────────────────────────────
    radar_short_lines = []
    for r in results_short[:7]:
        trend = get_score_trend(state, r.get("symbol",""), "SHORT")
        score = r.get("score_short", 0)
        s4h   = r.get("summary_4h","?")
        sym   = r.get("base_coin","?")
        oi_sc = r.get("oi_score_short", 0)
        oi_tag = f" OI{'+' if oi_sc>0 else ''}{oi_sc}" if oi_sc != 0 else ""
        fr    = r.get("funding_rate", 0)
        fr_tag = f" FR{fr:.3%}" if fr != 0 else ""
        dq    = r.get("data_quality_short", 1.0)
        dq_tag = f" ⚠️DQ{dq:.0%}" if dq < 1.0 else ""   # [v6.4.0 A9]
        radar_short_lines.append(f"  🔴 <b>{sym}</b> {score}/28 {trend} | {s4h}{oi_tag}{fr_tag}{dq_tag}")

    obs_long  = obs_long  or []
    obs_short = obs_short or []

    sem_tv = f"\n  ⚠️ Sem TV: {', '.join(tokens_sem_dados[:5])}" if tokens_sem_dados else ""

    # [v6.3.0 A4] Candle lock status line
    if candle_lock and candle_lock.get("use_prev"):
        cl_tag = (f"\n  ⚠️ Vela 15m em formação ({candle_lock['seconds_open']:.0f}s)"
                  f" — usando penúltima. Próx. fechamento: {candle_lock['next_close']:.0f}s")
    elif candle_lock:
        cl_tag = f"\n  ✅ Candle 15m ok ({candle_lock['seconds_open']:.0f}s, último: {candle_lock['ts_last_close']})"
    else:
        cl_tag = ""

    msg = (
        f"🤖 <b>Atirador v6.4.0</b> | {ts}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🌡️ <b>MERCADO</b>\n"
        f"  {fgi_ico} FGI: {fgi_val} ({fgi_txt}) | {btc_ico} BTC 4H: {ctx['btc']}\n"
        f"  📋 Thr LONG ≥{ctx['threshold']} ({ctx['verdict']})\n"
        f"  📋 Thr SHORT ≥{ctx['threshold_short']} ({ctx['verdict_short']})\n"
        f"  {op_ico} P&L: ${pnl_dia:+.2f} | Abertos: {n_abertos}/{MAX_TRADES_ABERTOS} | {slots} slot(s) livre(s)\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🔍 <b>PIPELINE</b>\n"
        f"  Universo: {total_items} brutos → {qualificados} qualificados\n"
        f"  LONG:  G4H={n_long_gate1} → G1H={n_long_gate2} → Top={len(results)} | Obs={len(obs_long)}\n"
        f"  SHORT: G4H={n_short_gate1} → G1H={n_short_gate2} → Top={len(results_short)} | Obs={len(obs_short)}\n"
        f"  📡 Fonte: {fonte} | ⏱️ {elapsed:.1f}s{sem_tv}{cl_tag}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
    )

    if radar_long_lines:
        msg += f"📈 <b>RADAR LONG</b> (top {len(radar_long_lines)})\n"
        msg += "\n".join(radar_long_lines) + "\n"
    else:
        msg += f"📈 <b>RADAR LONG</b>: nenhum token chegou ao score completo\n"

    msg += f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"

    if radar_short_lines:
        msg += f"📉 <b>RADAR SHORT</b> (top {len(radar_short_lines)})\n"
        msg += "\n".join(radar_short_lines) + "\n"
    else:
        msg += f"📉 <b>RADAR SHORT</b>: nenhum token chegou ao score completo\n"
...
def tg_notify(ctx: dict, results: list, results_short: list,
    ...
    """
    [v6.4.0] Orquestra notificações Telegram ao final de cada scan.

    Fluxo por rodada:
      1. Relatório de rodada (sempre) — contexto, pipeline, radar top-7, evolução score
      2. Alerta de call LONG  (apenas quando score ≥ threshold) — dados completos para operar
      3. Alerta de call SHORT (apenas quando score ≥ threshold_short) — dados completos

    A mensagem de relatório substituiu o heartbeat compacto — é mais informativa
    e mantém o objetivo de monitorar que o scanner está vivo.

    Falha de envio é silenciosa — WARNING no log, scan continua normalmente.
    """
    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID:
    ...
    # 1. Relatório de rodada (sempre enviado)
    if TELEGRAM_HEARTBEAT:
        msg_rel = _tg_relatorio_rodada(
        ...
        if _tg_send(msg_rel):
            n_enviados += 1
            LOG.info(f"  📲  Telegram relatório de rodada: enviado ✅")
        else:
            LOG.warning(f"  📵  Telegram relatório de rodada: falha no envio")

    # 2. Alertas LONG — mensagem separada por call
    alertas_long = [r for r in results if r["score"] >= ctx["threshold"]]
    for r in alertas_long:
        msg = _tg_call_long(r, ctx, pode_operar, state=state)
        if _tg_send(msg):
            n_enviados += 1
            LOG.info(f"  📲  Telegram LONG {r['base_coin']}: enviado ✅")
        else:
            LOG.warning(f"  📵  Telegram LONG {r['base_coin']}: falha no envio")

    # 3. Alertas SHORT — mensagem separada por call
    ...
    for r in alertas_short:
        ...
        if _tg_send(msg):
            n_enviados += 1
            LOG.info(f"  📲  Telegram SHORT {r['base_coin']}: enviado ✅")
        else:
            LOG.warning(f"  📵  Telegram SHORT {r['base_coin']}: falha no envio")

    total_tent = (1 if TELEGRAM_HEARTBEAT else 0) + len(alertas_long) + len(alertas_short)
...
def _load_telegram_config():
    config_file = "/tmp/atirador_telegram_config.json"
    
    # Tentar carregar do arquivo de configuração persistente
    if os.path.exists(config_file):
        try:
            ...
            pass  # Fallback para variáveis de ambiente
...
def save_telegram_config(token, chat_id):
    """Salva credenciais do Telegram no arquivo de configuração persistente."""
    config_file = "/tmp/atirador_telegram_config.json"
    config = {
    ...
    try:
        ...
        LOG.warning(f"  ⚠️  Erro ao salvar configuração Telegram: {e}")
        return False
...
#
# [v6.3.0 A3] Candles BEARISH adicionados ao COLS_15M.
# Bug anterior: score_candles(direction="SHORT") buscava Engulfing Bearish,
# Shooting Star, etc., mas esses campos nunca eram requisitados ao TradingView
# Scanner — P2 SHORT sempre retornava 0 por falta de dado, não por ausência
# de padrão. Corrigido adicionando os 6 campos bearish correspondentes.
COLS_15M = [
    "BB.upper|15", "BB.lower|15",        # P1 Bollinger — posição no canal
    "ATR|15",                             # Cálculo do SL dinâmico
    # P2 Candles LONG (bullish)
    ...
    # P2 Candles SHORT (bearish) — [v6.3.0 A3] adicionados para paridade real
...
def fetch_perpetuals():
    ...
    LOG.info("📡 [v6.4.0] Iniciando busca de tickers — hierarquia OKX → Gate.io → Bitget")
...
def calc_trade_params(price, atr, score=0):
    ...
    # Recalcular com alavancagem final (pode ser menor que a necessária)
    margem_real  = notional / alav_final        # margem efetivamente alocada
    risco_real   = margem_real * alav_final * stop_pct   # ≈ RISCO_POR_TRADE_USD
    ganho_rr2    = risco_real * RR_MINIMO

    stop_pct_pct = stop_pct * 100   # para exibição em %
    LOG.debug(f"    TradeParams LONG: SL={stop_pct_pct:.2f}% | "
              ...
              f"margem=${margem_real:.2f} | risco=${risco_real:.2f} | ganho=${ganho_rr2:.2f}")
    return {
        "direction"      : "LONG",
        "entry"          : price,
        "sl"             : sl,
...
def analyze_market_context(fg, btc_4h_str):
    ...
    # [v6.2.0] Threshold SHORT: espelho exato do LONG, lógica invertida.
    # A mesma leitura de mercado que torna LONG mais fácil torna SHORT mais difícil, e vice-versa.
    # Regras espelhadas (comparar com o bloco LONG acima):
    #   Bull Extremo (FGI≥80 + BTC BUY)  → SHORT desligado (99)      ← espelho: LONG desligado em Bear Extremo
    #   Medo Extremo (FGI≤20)            → SHORT CAUTELOSO (thr=20)  ← espelho: LONG CAUTELOSO (thr=20) em Medo
    #   Bear claro   (FGI≤30 + BTC SELL) → SHORT FAVORÁVEL (thr=14)  ← espelho: LONG FAVORÁVEL (thr=14) em Bull
    #   SELL presente ou FGI≥75          → SHORT MODERADO (thr=16)   ← espelho: LONG MODERADO (thr=20) em Bear
    #   Neutro                           → SHORT MODERADO (thr=20)   ← espelho: LONG MODERADO (thr=16) em Neutro
    if fg_val >= 80 and "BUY" in btc_4h_str:
        threshold_short = 99;  verdict_short = "DESLIGADO (Mercado Bull Extremo — short proibido)"
    elif fg_val <= 20:
        threshold_short = 20;  verdict_short = "CAUTELOSO (Medo Extremo — risco de squeeze)"
...
async def run_scan_async():
    ...
    LOG.info("🚀 Setup Atirador v6.4.0 | Arquitetura 3 Camadas (LONG+SHORT) | Iniciando scan...")
    t_start = time.time()
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if all_top:
            ...
            for d in top_full_long:
                ...
                if d.get("oi_estimado"):
                    ...
                    d["trade"] = calc_trade_params(d["price"], d.get("atr_15m", 0), score=sc)
        ...
        report  = f"{'='*58}\n"
        report += f"🎯 SETUP ATIRADOR v6.4.0\n"
        report += f"📅 {ts_full}\n"
        ...
        report += f"💼 GESTÃO DE RISCO — Sizing Risk-First [v6.4.0]\n"
        ...
        LOG.info(report)
        LOG.info(f"✅ Scan v6.4.0 concluído em {elapsed:.1f}s | Fonte: {DATA_SOURCE} | "
