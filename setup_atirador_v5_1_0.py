...
"""
=============================================================================
SETUP ATIRADOR v5.1 - Scanner Profissional de Criptomoedas
=============================================================================
...
v5.1 — Hierarquia de fontes robusta + Log de diagnóstico de fonte + Fixes:

  [v5.1 — NOVA HIERARQUIA DE FONTES DE TICKERS]
    Problema identificado nas rodadas de 21/03 e 22/03:
      - Bybit: timeout de 92s na rodada 1 (3 tentativas antes de responder)
      - Bybit: HTTP 403 Forbidden na rodada 2 (geo-block / rate-limit por IP)
    ...
    Nova hierarquia (ordem de tentativa):
      ...
      Fonte 2: OKX /market/tickers     — exchange mais liberal (600+ tokens)
      Fonte 3: Bitget (mantido)        — último recurso, funcional e estável
      Bybit: REMOVIDA                  — 2/2 falhas em produção

    Klines continuam sendo buscados da Bitget — nunca apresentou problema.

  [v5.1 — LOG DETALHADO DE DIAGNÓSTICO DE FONTE]
    Cada tentativa de fonte registra no LOG:
      - URL tentada, HTTP status, tempo de resposta
      - Motivo de falha (timeout / 403 / 404 / JSON inválido / vazio)
      - Fonte que efetivamente forneceu os dados
      - Total bruto recebido, qualificados, rejeitados por critério
    Relatório exibe fonte ativa e histórico de tentativas da execução.
    Objetivo: identificar rapidamente qual fonte está causando problemas.

  [v5.1 — FIX: Timeout reduzido para tickers]
    Bybit tinha timeout=20s por tentativa (3x = 60s possíveis de espera).
    Novo timeout para tickers de fonte primária: 8s.
    Se não responder em 8s, cai para próxima fonte imediatamente.
    Mantém timeout=20s para klines (downloads maiores, justificado).

  [v5.1 — FIX: Tokens bloqueados por Pump aparecem no relatório]
    Tokens que passam Gate 4H + Gate 1H mas são bloqueados por P7 (pump)
    no score parcial retornavam -99 e desapareciam silenciosamente.
    Agora aparecem em seção "🚫 BLOQUEADOS POR PUMP" no relatório e no LOG.

  [v5.1 — FIX: Aviso de RSI extremo no Gate 4H]
    RSI 4H > 80: aviso ⚠️ "Rally extremo" no LOG e no relatório.
    Token passa o gate normalmente, mas o aviso fica registrado.
    Contexto: RSI > 80 em 4H indica ativo muito estendido — risco de reversão.

  [v5.1 — FIX: Fallback de prefixo TV quando BYBIT: não retorna dados]
    Quando token não tem dados no TV com prefixo BYBIT:XUSDT.P,
    o sistema agora tenta BITGET:XUSDT.P antes de descartar o token.
    Reduz tokens sem dados TV especialmente no fallback Bitget.
...
Autor: Manus AI | v4.1→v5.1 (revisão Claude/Anthropic)
=============================================================================
"""
...
def _parse_coingecko_tickers(items):
    """
    Normaliza tickers do CoinGecko /derivatives para estrutura interna.

    CoinGecko /derivatives retorna contratos de múltiplas exchanges.
    Filtramos apenas perpetuals USDT (contract_type=perpetual, quote=USDT).
    Campos:
      symbol      → ex: "BTC_USDT" (normalizamos para "BTCUSDT")
      open_interest_usd → OI em USD direto
      volume_24h  → volume em USD
      funding_rate → taxa de financiamento decimal
      last_price  → preço atual
      price_change_percentage_24h → variação em %
    """
    qualified  = []
    rej_symbol = rej_vol = rej_oi = rej_type = 0
    for t in items:
        # Filtrar apenas perpetuals
        if t.get("contract_type", "").lower() not in ("perpetual", ""):
            rej_type += 1; continue
        sym = t.get("symbol", "").replace("/", "").replace("-", "").replace("_", "")
        if not sym.endswith("USDT"):
            rej_symbol += 1; continue
        turnover = sf(t.get("volume_24h", 0))
        if turnover < MIN_TURNOVER_24H:
            rej_vol += 1; continue
        oi_usd = sf(t.get("open_interest_usd", 0))
        if oi_usd < MIN_OI_USD:
            rej_oi += 1; continue
        price = sf(t.get("last_price", 0) or t.get("index", 0))
        if price <= 0:
            rej_symbol += 1; continue
...
def fetch_perpetuals():
    ...
    LOG.info("📡 [v5.1] Iniciando busca de tickers — hierarquia de 3 fontes")
    ...
    # ------------------------------------------------------------------
    # FONTE 1: CoinGecko /derivatives
    # Agregador neutro — retorna perpetuals de múltiplas exchanges.
    # Endpoint público, sem autenticação, sem geo-block conhecido.
    # Rate limit free tier: 30 req/min (suficiente: 1 req/execução).
    # ------------------------------------------------------------------
    ...
    if resultado:
        ...
        LOG.info(f"  ✅  [CoinGecko] Fonte ativa | {total} brutos → {len(qualified)} qualificados")
...
async def run_scan_async():
    ...
    LOG.info("🚀 Setup Atirador v5.1 | Arquitetura 3 Camadas | Iniciando scan...")
    t_start = time.time()
    ...
    async with aiohttp.ClientSession() as session:
        ...
        LOG.info(report)
        LOG.info(f"✅ Scan v5.1 concluído em {elapsed:.1f}s | Fonte: {DATA_SOURCE} | "
