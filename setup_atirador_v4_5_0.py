...
"""
=============================================================================
SETUP ATIRADOR v4.5 - Scanner Profissional de Criptomoedas
=============================================================================
...
v4.5 vs v4.3:
  [BUG CRÍTICO CORRIGIDO] Klines da Bitget retornavam vazios por erro de
  encoding Brotli. O aiohttp envia Accept-Encoding com "br" por padrão;
  sem o pacote brotli instalado, a decodificação falha silenciosamente
  no except e retorna []. Correções aplicadas:
    1. Sessão aiohttp separada para Bitget com Accept-Encoding: gzip, deflate
       (desabilita Brotli na negociação HTTP)
    2. fetch_klines_async usa resp.read() + json.loads() em vez de resp.json()
       para controle explícito da decodificação
    3. Logs de erro explícitos nos fetches de klines (não mais except silencioso)
    4. Fallback síncrono via requests para klines se sessão async falhar
...
Autor: Manus AI | v4.1 → v4.2 → v4.3 → v4.5 (revisão Claude/Anthropic)
=============================================================================
"""
