...
"""
=============================================================================
SETUP ATIRADOR v5.3 - Scanner Profissional de Criptomoedas
=============================================================================
...
v5.3 — Gate.io como Fonte 1 (substitui CoinGecko definitivamente):

  [v5.3 — GATE.IO COMO FONTE 1]
    Histórico de tentativas de Fonte 1:
      v5.0: Bybit        → geo-block Brasil (timeout 92s + HTTP 403)
      v5.1: CoinGecko    → 21.094 itens, todos rejeitados (parser incompatível)
      v5.2: CoinGecko/exchange-specific → HTTP 404 (IDs incorretos + endpoint pago)
      v5.3: Gate.io      → confirmada acessível do Brasil, dados válidos ✅

    Pesquisa exaustiva concluiu (22/03/2026):
      - Binance fapi: geo-block Brasil confirmado (igual à Bybit)
      - CoinGlass: pago (sem free tier com acesso à API)
      - Laevitas, The Block, Velo Data: enterprise/pagos
      - Coinalyze: máx 20 símbolos por chamada (inviável)
      - Não existe fonte gratuita de dados AGREGADOS acessível do Brasil.
    Decisão: usar a melhor fonte LOCAL disponível = Gate.io.

    Gate.io vantagens sobre OKX (fonte anterior):
      - Distribuição de volume mais equilibrada (menos distorção por meme coins)
      - OI em contratos com quanto_multiplier → cálculo USD mais preciso
      - volume_24h_quote já em USDT (sem conversão)
      - funding_rate no mesmo endpoint dos tickers

    Implementação:
      - 2 chamadas: /futures/usdt/tickers + /futures/usdt/contracts
      - /contracts buscado 1x com cache de 24h (quanto_multiplier muda raramente)
      - Normalização BTC_USDT → BTCUSDT (crítico para TV Scanner e klines)
      - OI USD = total_size × mark_price × quanto_multiplier

    Hierarquia final: Gate.io → OKX → Bitget

  [v5.3 — REMOÇÃO DEFINITIVA DE COINGECKO]
    CoinGecko nunca funcionou em produção (v5.1 e v5.2).
    Código removido completamente — sem tentativas desnecessárias.
    Economia: ~6s por execução que eram gastos aguardando HTTP 404.
...
Autor: Manus AI | v4.1→v5.3 (revisão Claude/Anthropic)
=============================================================================
"""
...
def fetch_perpetuals():
    """
    [v5.3] Busca perpetuals USDT com hierarquia Gate.io → OKX → Bitget.

    Gate.io (Fonte 1): confirmada acessível do Brasil (teste 22/03/2026).
      Requer 2 chamadas: tickers + contracts (contracts cacheado 24h).
      OI calculado com precisão via quanto_multiplier.
      Normaliza BTC_USDT → BTCUSDT para compatibilidade com TV/klines.

    OKX (Fonte 2): fallback já funcional desde v5.1.
    Bitget (Fonte 3): último recurso, estável desde v4.x.

    CoinGecko: REMOVIDA definitivamente (nunca funcionou em produção).
    Bybit/Binance: geo-block Brasil confirmado — não implementadas.
    """
    global DATA_SOURCE, DATA_SOURCE_ATTEMPTS
    DATA_SOURCE_ATTEMPTS = []

    LOG.info("📡 [v5.3] Iniciando busca de tickers — hierarquia Gate.io → OKX → Bitget")
    LOG.info(f"   Filtros: vol≥${MIN_TURNOVER_24H/1e6:.1f}M | OI≥${MIN_OI_USD/1e6:.0f}M | timeout={TICKER_TIMEOUT}s/fonte")

    # ------------------------------------------------------------------
    # FONTE 1: Gate.io (confirmada BR ✅ — substituiu CoinGecko na v5.3)
    # 2 chamadas: tickers (dados) + contracts (quanto_multiplier, cache 24h)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # FONTE 2: OKX — fallback funcional desde v5.1
    # ------------------------------------------------------------------
    resultado = _try_source(
        nome       = "OKX",
    ...
    # ------------------------------------------------------------------
    LOG.warning("  ⚠️  [v5.3] Gate.io e OKX indisponíveis — usando Bitget (último recurso)")
    resultado = _try_source(
        nome       = "Bitget",
    ...
    LOG.error("    2. Testar manualmente: curl https://api.gateio.ws/api/v4/futures/usdt/tickers")
...
async def run_scan_async():
    ...
    LOG.info("🚀 Setup Atirador v5.3 | Arquitetura 3 Camadas | Iniciando scan...")
    t_start = time.time()
    ...
    async with aiohttp.ClientSession() as session:
        ...
        LOG.info(report)
        LOG.info(f"✅ Scan v5.3 concluído em {elapsed:.1f}s | Fonte: {DATA_SOURCE} | "
