...
"""
=============================================================================
SETUP ATIRADOR v6.1.2 - Scanner Profissional de Criptomoedas
=============================================================================
...
Autor: Manus AI | v4.1→v6.1.2 (revisão Claude/Anthropic)
=============================================================================
...
def _tg_call_long(r: dict, ctx: dict, pode_operar: bool) -> str:
    """
    Formata mensagem de ALERTA LONG para o Telegram.
    Campos reais da estrutura do Atirador 6.x.
    """
    t      = r["trade"]
    score  = r["score"]
    sym    = r["base_coin"]
    ...
    # Razões técnicas (máx 3 para caber no celular)
    razoes = " | ".join(r.get("reasons", [])[:3])

    msg = (
        f"🚀 <b>LONG {sym}</b>{bloq}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"📊 Score: <b>{score}/26</b> (thr≥{thr}) | 4H: {r.get('summary_4h','?')} | 1H: {r.get('summary_1h','?')}\n"
        f"💡 {razoes}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        ...
        f"⚡ Alav: <b>{t['alavancagem']}x</b> | Risco: ${t['risco_usd']:.2f} | Ganho: ${t['ganho_rr2_usd']:.2f}\n"
        f"📈 Vol 24h: ${r['turnover_24h']/1e6:.1f}M | FR: {r.get('funding_rate',0):.4%}\n"
        f"⏰ Trailing: ativar no TP1 → SL breakeven +0.5%"
    )
    return msg


def _tg_call_short(r: dict, ctx: dict, pode_operar: bool) -> str:
    """
    Formata mensagem de ALERTA SHORT para o Telegram.
    """
    t      = r["trade_short"]
    ...
    razoes = " | ".join(r.get("reasons_short", [])[:3])

    msg = (
        f"📉 <b>SHORT {sym}</b>{bloq}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"📊 Score: <b>{score}/26</b> (thr≥{thr}) | 4H: {r.get('summary_4h','?')} | 1H: {r.get('summary_1h','?')}\n"
        f"💡 {razoes}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
...
def _tg_heartbeat(ctx: dict, total_items: int, n_long_gate1: int, n_long_gate2: int,
                  n_short_gate1: int, n_short_gate2: int, results: list,
                  results_short: list, fonte: str, elapsed: float,
                  pode_operar: bool, pnl_dia: float, n_abertos: int) -> str:
    """
    Formata mensagem de HEARTBEAT — resumo compacto de cada rodada.

    Objetivo duplo:
      1. Monitoramento: confirma que o scanner está vivo e funcionando
      2. Consistência: score máximo e pipeline counts permitem detectar
         anomalias (ex: universo caiu de 100 para 5 tokens de repente)

    Sempre enviado, independente de haver alertas.
    """
    ts      = datetime.now(BRT).strftime("%d/%m %H:%M")
    fgi_ico = "🔴" if ctx["fg"] <= 20 else ("🟡" if ctx["fg"] <= 45 else ("🟢" if ctx["fg"] >= 60 else "⚪"))
    btc_ico = "📈" if "BUY" in ctx["btc"] else ("📉" if "SELL" in ctx["btc"] else "➡️")
    op_ico  = "✅" if pode_operar else "🛑"

    # Score máximo encontrado (LONG e SHORT)
    max_long  = max((r["score"] for r in results), default=0)
    max_short = max((r.get("score_short", 0) for r in results_short), default=0)

    # Determinar se há alertas
    ...
    # [v6.1.2] Detectar duplicatas (mesmo token em LONG e SHORT — não deveria ocorrer
    # pela regra de exclusividade, mas loga se acontecer para debug)
    long_syms  = {r["symbol"] for r in alertas_long}
    short_syms = {r["symbol"] for r in alertas_short}
    duplicatas = long_syms & short_syms
    if duplicatas:
        LOG.warning(f"  ⚠️  Heartbeat: duplicatas LONG+SHORT detectadas: {duplicatas}")

    # Linha de alertas
    if alertas_long or alertas_short:
        calls = []
        for r in alertas_long:  calls.append(f"🚀{r['base_coin']}({r['score']})")
        for r in alertas_short: calls.append(f"📉{r['base_coin']}({r.get('score_short',0)})")
        alerta_line = f"🔥 CALLS: {' '.join(calls)}"
    else:
        alerta_line = f"💤 Sem calls (L max={max_long}/26 | S max={max_short}/26)"

    msg = (
        f"🤖 <b>Atirador v6.1.2</b> | {ts}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"{fgi_ico} FGI: {ctx['fg']} | {btc_ico} BTC: {ctx['btc']}\n"
        f"📋 Thr LONG≥{ctx['threshold']} | SHORT≥{ctx['threshold_short']}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"🔍 Universo: {total_items} → qualif.\n"
        f"   LONG:  G4H={n_long_gate1} | G1H={n_long_gate2} | analisados={len(results)}\n"
        f"   SHORT: G4H={n_short_gate1} | G1H={n_short_gate2} | analisados={len(results_short)}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"{alerta_line}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"{op_ico} P&L: ${pnl_dia:+.2f} | Abertos: {n_abertos}/2\n"
        f"📡 Fonte: {fonte} | ⏱ {elapsed:.0f}s"
    )
    return msg


def tg_notify(ctx: dict, results: list, results_short: list,
              total_items: int, n_long_gate1: int, n_long_gate2: int,
              n_short_gate1: int, n_short_gate2: int,
              fonte: str, elapsed: float,
              pode_operar: bool, pnl_dia: float, n_abertos: int):
    """
    [v6.1.2] Orquestra todas as notificações Telegram ao final do scan.

    Fluxo:
      1. Envia ALERTA para cada call LONG com score ≥ threshold
      2. Envia ALERTA para cada call SHORT com score ≥ threshold_short
      3. Envia HEARTBEAT se TELEGRAM_HEARTBEAT=True (toda rodada)

    Chamado uma vez no final de run_scan_async, após gerar results.
    Falha de envio é silenciosa — WARNING no log, scan continua.
    """
    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID:
    ...
    for r in alertas_long:
        msg = _tg_call_long(r, ctx, pode_operar)
        if _tg_send(msg):
            n_enviados += 1
    ...
    for r in alertas_short:
        msg = _tg_call_short(r, ctx, pode_operar)
        if _tg_send(msg):
            n_enviados += 1
    ...
    # 3. Heartbeat (sempre, independente de alertas)
    if TELEGRAM_HEARTBEAT:
        msg = _tg_heartbeat(
            ctx, total_items, n_long_gate1, n_long_gate2,
            n_short_gate1, n_short_gate2, results, results_short,
            fonte, elapsed, pode_operar, pnl_dia, n_abertos
        )
        if _tg_send(msg):
            n_enviados += 1
    ...
    total_tentativas = len(alertas_long) + len(alertas_short) + (1 if TELEGRAM_HEARTBEAT else 0)
    LOG.info(f"  📲  Telegram: {n_enviados}/{total_tentativas} mensagens enviadas")
...
#
TELEGRAM_TOKEN   = os.getenv("TELEGRAM_TOKEN", "")    # export TELEGRAM_TOKEN="7xxx:AAF..."
TELEGRAM_CHAT_ID = os.getenv("TELEGRAM_CHAT_ID", "")  # export TELEGRAM_CHAT_ID="123456789"
...
def fetch_perpetuals():
    """
    [v6.1.2] Busca perpetuals USDT — hierarquia OKX → Gate.io → Bitget.
    ...
    LOG.info("📡 [v6.1.2] Iniciando busca de tickers — hierarquia OKX → Gate.io → Bitget")
...
def analyze_market_context(fg, btc_4h_str):
    ...
    # [v6.0/v6.1.2] Threshold SHORT: inverso do LONG
...
async def run_scan_async():
    ...
    LOG.info("🚀 Setup Atirador v6.1.2 | Arquitetura 3 Camadas (LONG+SHORT) | Iniciando scan...")
    t_start = time.time()
    ...
    async with aiohttp.ClientSession() as session:
        ...
        report  = f"{'='*58}\n"
        report += f"🎯 SETUP ATIRADOR v6.1.2\n"
        report += f"📅 {ts_full}\n"
        ...
        LOG.info(report)
        LOG.info(f"✅ Scan v6.1.2 concluído em {elapsed:.1f}s | Fonte: {DATA_SOURCE} | "
