...
"""
=============================================================================
SETUP ATIRADOR v4.6 - Scanner Profissional de Criptomoedas
=============================================================================
...
v4.6 vs v4.5 (problemas remanescentes do relatório de avaliação):
  [FIX 1] Versão corrigida no header do relatório (era v4.3, agora v4.6)
  [FIX 2] calculate_score: gate 4H SELL corrigido (STRONG_SELL também descartava
          incorretamente tokens NEUTRAL). Condição reescrita com clareza.
  [FIX 3] Loop de análise: debug logs explícitos por token (score, trade_params,
          klines disponíveis). Tokens não somem mais silenciosamente.
  [FIX 4] results.append: token entra em results com qualquer score ≥ 0 desde
          que trade_params seja válido. Antes: token com score baixo não aparecia
          nem em Oportunidades porque nunca entrava na lista results.
  [FIX 5] Relatório: threshold de Oportunidades ajustado. Nova seção "Radar"
          para scores 5–9 (análise completa mas sinal fraco). Em Bear Market
          o score máximo sem klines 4H/1H é ~11 pts — o threshold antigo de 10
          excluía tokens válidos.
  [FIX 6] Seção Bear Market expandida com mensagem explicativa.

v4.5 vs v4.3:
  [BUG CRÍTICO] Encoding Brotli em aiohttp resolvido (klines retornavam vazios)
  BITGET_HEADERS com Accept-Encoding: gzip, deflate
  read() + json.loads() explícito
  Logs de erro nos fetches de klines

v4.3 vs v4.2:
  Gate principal migrado de 4H para 1H (velocidade para scalp)
  4H vira modificador de contexto (SELL descarta, resto segue)
  6 pilares redundantes removidos
  Novo pilar P-1H: suporte/OB em klines 1H
  Símbolos corrigidos: TradingView exige BTCUSDT, não BTC

Autor: Manus AI | v4.1 → v4.2 → v4.3 → v4.5 → v4.6 (revisão Claude/Anthropic)
=============================================================================
"""
...
async def api_get_async(session, url, retries=3, headers=None):
    """GET assíncrono com decodificação explícita para evitar erro Brotli."""
    for i in range(retries):
        try:
            async with session.get(url, timeout=20, headers=headers) as resp:
                resp.raise_for_status()
                # Usa read() + json.loads() em vez de resp.json() para
                # controlar encoding explicitamente e evitar falha silenciosa
                raw = await resp.read()
            ...
            if i < retries - 1:
                ...
                print(f"  ⚠️  api_get_async falhou ({url[:60]}...): {e}")
                return None

def api_get(url, retries=3):
    for i in range(retries):
        try:
            resp = requests.get(url, timeout=20, headers={"Accept-Encoding": "gzip, deflate"})
            resp.raise_for_status()
            return resp.json()
        except Exception as e:
...
async def fetch_klines_async(session, symbol, granularity="15m", limit=60):
    """
    Busca klines da Bitget com headers explícitos anti-Brotli.
    [v4.5 FIX] Usa BITGET_HEADERS para desabilitar encoding Brotli.
    Loga erro explicitamente em vez de retornar [] silenciosamente.
    """
    try:
        ...
        if not data:
            print(f"  ⚠️  Klines {symbol} {granularity}: resposta vazia")
            return []
        if "data" not in data:
            print(f"  ⚠️  Klines {symbol} {granularity}: campo 'data' ausente — {str(data)[:80]}")
            return []
        ...
        print(f"  ❌  fetch_klines_async {symbol} {granularity}: {e}")
        return []

async def fetch_klines_cached_async(session, symbol, granularity="4H", limit=60):
    """Klines com cache local. Usa fetch_klines_async corrigido."""
...
def calculate_score(d, candles_15m=None, candles_1h=None, candles_4h=None, fg_value=50):
    ...
    Retornos especiais:
      ...
      ≥0  = Score válido (incluindo scores baixos — não descartar aqui)

    IMPORTANTE: max(sc, 0) foi removido do return. Score pode ser 0 após
    penalidades. O filtro de exibição no relatório cuida do threshold.
    """
    sc       = 0
    reasons  = []
    breakdown = []
    ...
    final_sc = max(sc, 0)  # Ainda evita negativo pós-penalidades, mas sem confundir com -1
...
async def run_scan_async():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if not gate2_passed:
            ...
            print(report)
            output_path = "/tmp/ultimo_scan_atirador_v46.txt"
        ...
        if top_full:
            ...
            for i, d in enumerate(top_full):
                ...
                # [v4.6] Log explícito se klines faltarem — ajuda diagnóstico
                if not k15m:
                    print(f"  ⚠️  {sym}: klines 15m vazios — pulando")
                    continue
                if not k1h:
                    print(f"  ⚠️  {sym}: klines 1H vazios — pilares 1H serão 0")
                if not k4h:
                    print(f"  ⚠️  {sym}: klines 4H vazios — pilares 4H serão 0")
                ...
                # [v4.6] Debug log do score de cada token qualificado
                trade = calc_trade_params(d["price"], d.get("atr_15m", 0))
                trade_ok = trade is not None
                print(f"  📊 {sym}: score={sc}/26 | trade_params={'✅' if trade_ok else '❌ ATR inválido'}")

                # [v4.6 FIX] Adiciona a results mesmo com score baixo, desde que
                # trade_params seja válido. O relatório filtra por threshold.
                # Antes: tokens com score < threshold sumiam silenciosamente.
                if trade_ok:
                    d["trade"] = trade
                    results.append(d)
                else:
                    print(f"  ⚠️  {sym}: trade_params None (ATR={d.get('atr_15m', 0):.4f}) — não entra no relatório")
        ...
        report  = f"🎯 SETUP ATIRADOR v4.6 — {ts}\n"
        report += f"{'='*58}\n"
