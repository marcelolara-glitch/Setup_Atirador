...
"""
=============================================================================
SETUP ATIRADOR v4.7 - Scanner Profissional de Criptomoedas
=============================================================================
...
Score máximo: 26 pts
Thresholds: Favorável ≥16 | Moderado ≥18 | Cauteloso(Bear) ≥20

v4.7 — Sistema de Log Robusto + Cache Corrigido:
  [LOG 1]  Logger centralizado: grava em arquivo + terminal simultaneamente
           Arquivo: /tmp/atirador_YYYYMMDD_HHMM.log (um por execução)
  [LOG 2]  Cache: loga origem (HIT/MISS/CORROMPIDO), idade e qtd de candles
  [LOG 3]  API Bitget: loga URL, status HTTP, tamanho da resposta
  [LOG 4]  API TradingView: loga batch size, colunas, tokens retornados
  [LOG 5]  Gates: loga cada token individualmente (passou/rejeitado + motivo)
  [LOG 6]  Score: breakdown completo por pilar para cada token analisado
  [LOG 7]  Trade params: entry, SL%, ATR, alavancagem calculada
  [LOG 8]  Erros de cálculo: divisão por zero, valores None, ATR inválido
  [CACHE 1] Não grava cache se klines retornarem [] (evita cache corrompido)
  [CACHE 2] Invalida cache existente se contiver lista vazia
  [CACHE 3] Loga sempre: arquivo de cache, idade, candles encontrados
  [FIX]    Print inicial corrigido para v4.7

v4.6: fixes de relatório, gate SELL, threshold oportunidades, debug logs
v4.5: fix encoding Brotli (klines retornavam vazios)
v4.3: gate 1H como principal, remoção de redundâncias, símbolos TV corrigidos

Autor: Manus AI | v4.1→v4.7 (revisão Claude/Anthropic)
=============================================================================
"""
...
def setup_logger():
    ...
    logfile = f"{LOG_DIR}/atirador_{ts}.log"

    logger = logging.getLogger("atirador")
    logger.setLevel(logging.DEBUG)
    logger.handlers.clear()

    # Formato completo para arquivo (com timestamp e nível)
    ...
    ch.setLevel(logging.INFO)   # Terminal: INFO e acima
...
# Logger global — inicializado em main()
LOG  = None
LOG_FILE = None
...
async def fetch_tv_batch_async(session, symbols, columns, retries=3):
    ...
    Loga: batch size, colunas, tokens retornados, tokens ausentes, valores None.
    symbols: lista de símbolos COMPLETOS ex: ["BTCUSDT"] → monta "BYBIT:BTCUSDT"
    """
    ...
    LOG.debug(f"  TV batch: {len(symbols)} tokens | cols: {columns}")
...
async def fetch_klines_async(session, symbol, granularity="15m", limit=60):
    ...
    try:
        ...
        # Log de diagnóstico: período coberto pelos candles
...
def calculate_score(d, candles_15m=None, candles_1h=None, candles_4h=None, fg_value=50):
    ...
    # [v4.7 LOG] Breakdown completo de pilares no arquivo de log (DEBUG)
    sym = d.get("base_coin", "?")
...
async def run_scan_async():
    ...
    LOG.info("🚀 Setup Atirador v4.7 | Arquitetura 3 Camadas | Iniciando scan...")
    t_start = time.time()
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for d in perpetuals[:TOP_N]:
            ...
            if "SELL" in s4h:
                gate1_rejected += 1
                LOG.debug(f"  ❌  {d['base_coin']:<8} 4H={s4h} (val={raw_val}) RSI={rsi_4h:.1f} — REJEITADO")
            else:
                gate1_passed.append(d)
                LOG.debug(f"  ✅  {d['base_coin']:<8} 4H={s4h} (val={raw_val}) RSI={rsi_4h:.1f} — PASSOU")

        LOG.info(f"  Gate 4H: {len(gate1_passed)} passaram | {gate1_rejected} rejeitados (SELL)")
        ...
        for d in gate1_passed:
            ...
            if "BUY" not in s1h:
                gate2_rejected += 1
                LOG.debug(f"  ❌  {d['base_coin']:<8} 1H={s1h} (val={raw_1h}) — REJEITADO")
            else:
                gate2_passed.append(d)
                LOG.debug(f"  ✅  {d['base_coin']:<8} 1H={s1h} (val={raw_1h}) — PASSOU")

        LOG.info(f"  Gate 1H: {len(gate2_passed)} passaram | {gate2_rejected} rejeitados (não BUY)")
        ...
        if not gate2_passed:
            ...
            output_path = "/tmp/ultimo_scan_atirador_v47.txt"
        ...
        log_section("ETAPA 3b — Score parcial (sem klines)")
        for d in gate2_passed:
            sc_p, _, _ = calculate_score(d, fg_value=fg.get("value", 50))
            d["_partial_score"] = sc_p
            LOG.debug(f"  {d['base_coin']:<8} score parcial: {sc_p}/26")
