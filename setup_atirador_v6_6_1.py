...
"""
=============================================================================
SETUP ATIRADOR v6.6.1 - Scanner Profissional de Criptomoedas
=============================================================================
...
Autor: Manus AI | v4.1→v6.6.1 (revisão Claude/Anthropic)
=============================================================================
...
def _fmt_pilares_call(bd: list) -> str:
    """
    [v6.6.1] Formata breakdown de pilares para msgs de Call e QUASE.
    Reutilizado por _tg_call_long, _tg_call_short e _tg_quase.
    """
    linhas = []
    for pilar, pts, max_pts, detalhe in bd:
        if max_pts == 0:
            continue   # P7 Pump — skip
        ico_p = "✅" if pts > 0 else ("🔻" if pts < 0 else "⬜")
        desc  = detalhe.split("|")[0].strip()
        if len(desc) > 44:
            desc = desc[:42] + "…"
        linhas.append(f"  {ico_p} {pilar:<18} {pts:>+2}/{max_pts}  {desc}")
    return "\n".join(linhas) if linhas else "  (breakdown indisponível)"


def _tg_call_long(r: dict, ctx: dict, pode_operar: bool, state: dict = None) -> str:
    """
    [v6.6.1] Mensagem exclusiva de CALL LONG — parâmetros operacionais completos
    + breakdown de todos os pilares. Enviada separada do heartbeat.
    """
    t      = r["trade"]
    score  = r["score"]
    sym    = r["base_coin"]
    ...
    bd     = r.get("breakdown", [])
    margem_tag = "⚠️ MARGEM ACIMA DO LIMITE" if t.get("margem_excedida") else "Margem ok"

    msg  = f"🚀 <b>LONG {sym}</b>{bloq}  {trend}  {score}/28\n"
    msg += f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += f"  {r.get('summary_4h','?')} 4H  |  {r.get('summary_1h','?')} 1H  |  score ≥{thr}\n"
    msg += f"  Vol {r['turnover_24h']/1e6:.1f}M  |  OI {oi_str}  |  FR {r.get('funding_rate',0):.4%}\n"
    msg += f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += f"📍 Entrada  <b>${t['entry']:.4f}</b>\n"
    msg += f"🎯 TP1  ${t['tp1']:.4f}  (+{t['sl_distance_pct']:.2f}%) → fechar 50%\n"
    msg += f"🎯 TP2  ${t['tp2']:.4f}  (+{t['sl_distance_pct']*2:.2f}%) → fechar 30%\n"
    msg += f"🎯 TP3  ${t['tp3']:.4f}  (+{t['sl_distance_pct']*3:.2f}%) → fechar 20%\n"
    msg += f"🛑 SL   ${t['sl']:.4f}  (-{t['sl_distance_pct']:.2f}%)\n"
    msg += f"⚡ <b>{t['alavancagem']}x</b>  |  Notional ${t.get('notional_usd',0):.0f}  |  Margem ${t.get('margem_usd',0):.0f}  |  {margem_tag}\n"
    msg += f"   Risco ${t['risco_usd']:.2f}  |  Ganho (RR1:2) ${t['ganho_rr2_usd']:.2f}\n"
    msg += f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += f"🧱 <b>PILARES ({score}/28)</b>\n"
    msg += _fmt_pilares_call(bd) + "\n"
    msg += f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += f"⏰ Mover SL para breakeven +0.5% após TP1"
    return msg


def _tg_call_short(r: dict, ctx: dict, pode_operar: bool, state: dict = None) -> str:
    """
    [v6.6.1] Mensagem exclusiva de CALL SHORT — parâmetros operacionais completos
    + breakdown de todos os pilares. Enviada separada do heartbeat.
    """
    t      = r["trade_short"]
    ...
    msg  = f"📉 <b>SHORT {sym}</b>{bloq}  {trend}  {score}/28\n"
    msg += f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += f"  {r.get('summary_4h','?')} 4H  |  {r.get('summary_1h','?')} 1H  |  score ≥{thr}\n"
    msg += f"  Vol {r['turnover_24h']/1e6:.1f}M  |  OI {oi_str}  |  FR {r.get('funding_rate',0):.4%}\n"
    msg += f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
    ...
    msg += f"🎯 TP2  ${t['tp2']:.4f}  (-{t['sl_distance_pct']*2:.2f}%) → fechar 30%\n"
    msg += f"🎯 TP3  ${t['tp3']:.4f}  (-{t['sl_distance_pct']*3:.2f}%) → fechar 20%\n"
    msg += f"🛑 SL   ${t['sl']:.4f}  (+{t['sl_distance_pct']:.2f}%) ← ACIMA\n"
    msg += f"⚡ <b>{t['alavancagem']}x</b>  |  Notional ${t.get('notional_usd',0):.0f}  |  Margem ${t.get('margem_usd',0):.0f}  |  {margem_tag}\n"
    msg += f"   Risco ${t['risco_usd']:.2f}  |  Ganho (RR1:2) ${t['ganho_rr2_usd']:.2f}\n"
    msg += f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += f"🧱 <b>PILARES ({score}/28)</b>\n"
...
def _tg_relatorio_rodada(ctx: dict, total_items: int, qualificados: int,
    ...
    """
    [v6.6.1] Heartbeat — status do mercado e preparação para possíveis calls.

    Autocontido em 1 mensagem Telegram (≤ 4096 chars). Sem breakdown de pilares
    — esse nível de detalhe vai nas mensagens QUASE e de Call, que são separadas.

    Estrutura:
      1. Cabeçalho     — versão, timestamp
      2. Mercado        — FGI, BTC 4H, threshold, P&L, slots, candle lock
      3. Pipeline       — universo → gates → análise, fonte, tempo
      4. Radar LONG     — top-5 com score e tendência, distância do threshold
      5. Radar SHORT    — idem
      6. Veredicto      — AGUARDAR / CALLS DISPARADAS (referência às msgs separadas)
    """
    ...
    fgi_txt = ("Medo Extremo" if fgi_val <= 20 else "Medo" if fgi_val <= 40 else
               "Neutro"       if fgi_val <= 60 else "Ganância" if fgi_val <= 80
    ...
    if candle_lock and candle_lock.get("use_prev"):
        cl_tag = f"  ⚠️ Candle em formação ({candle_lock['seconds_open']:.0f}s) — penúltimo usado\n"
    elif candle_lock:
        cl_tag = f"  ✅ Candle 15m ok ({candle_lock['seconds_open']:.0f}s após {candle_lock['ts_last_close']})\n"
    else:
        cl_tag = ""
    ...
    msg += f"  {op_ico} P&L <b>${pnl_dia:+.2f}</b>  |  Slots {slots}/{MAX_TRADES_ABERTOS}\n"
    msg += cl_tag

    # ── 3. PIPELINE ──────────────────────────────────────────────────────────
    sem_tv = f"  ⚠️ Sem TV: {', '.join(tokens_sem_dados[:3])}\n" if tokens_sem_dados else ""
    obs_l  = len(obs_long  or [])
    obs_s  = len(obs_short or [])
    msg += f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += f"🔍 <b>PIPELINE</b>\n"
    msg += f"  Universo: {total_items} tokens  →  {qualificados} qualificados\n"
    msg += f"  Gate 4H:  LONG {n_long_gate1:<4} |  SHORT {n_short_gate1}\n"
    msg += f"  Gate 1H:  LONG {n_long_gate2:<4} |  SHORT {n_short_gate2}\n"
    msg += f"  Análise:  LONG {len(results):<4} |  SHORT {len(results_short)}  |  Obs {obs_l}L {obs_s}S\n"
    msg += f"  Fonte: {fonte}  |  ⏱️ {elapsed:.1f}s\n"
    msg += sem_tv

    # ── 4+5. RADAR ───────────────────────────────────────────────────────────
    def _radar_line(lista, direction, thr, n=5):
        key    = "score" if direction == "LONG" else "score_short"
        max_sc = max((r[key] if direction == "LONG" else r.get(key,0) for r in lista), default=0)
        falta  = thr - max_sc
        ...
        for r in lista[:n]:
            ...
            sym = r.get("base_coin","?")
            parts.append(f"{sym} {sc}{tr}")
        tokens_str = "  ".join(parts) if parts else "—"
        return (f"{ico} <b>{label}</b>  máx {max_sc}/28 — faltam {falta} pts (thr={thr})\n"
                f"  {tokens_str}")

    msg += f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += _radar_line(results,       "LONG",  thr_l) + "\n\n"
    msg += _radar_line(results_short, "SHORT", thr_s) + "\n"

    # ── 6. VEREDICTO ─────────────────────────────────────────────────────────
    msg += f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
    if not pode_operar:
        msg += f"🛑 <b>BOT OFF</b> — {ctx.get('verdict','sem operações')}\n"
    elif alertas_long or alertas_short:
        n_calls = len(alertas_long) + len(alertas_short)
        msg += f"🔥 <b>{n_calls} CALL(S) DISPARADA(S)</b> — ver mensagem(ns) exclusiva(s) abaixo\n"
    else:
        # Verificar QUASE (faltam <= 5 pts) para indicar no heartbeat
        quase_long  = [r for r in results
                       if 0 < thr_l - r["score"] <= 5]
        quase_short = [r for r in results_short
                       if 0 < thr_s - r.get("score_short",0) <= 5]
        n_quase = len(quase_long) + len(quase_short)
        if n_quase > 0:
            nomes = []
            for r in sorted(quase_long,  key=lambda x: thr_l - x["score"]):
                nomes.append(f"LONG {r['base_coin']} ({thr_l - r['score']} pts)")
            for r in sorted(quase_short, key=lambda x: thr_s - x.get("score_short",0)):
                nomes.append(f"SHORT {r['base_coin']} ({thr_s - r.get('score_short',0)} pts)")
            msg += f"🟡 <b>{n_quase} QUASE</b> — ver mensagem(ns) de alerta abaixo\n"
            msg += "  " + "  |  ".join(nomes) + "\n"
        else:
            msg += f"⚪ <b>AGUARDAR</b> — sem candidatos próximos do threshold\n"

    # Guardar limite
    LIMITE = 4090
    if len(msg) > LIMITE:
        linhas = msg.split("\n")
        while len("\n".join(linhas)) > LIMITE and len(linhas) > 10:
            linhas.pop(-2)
        msg = "\n".join(linhas)
...
def _tg_quase(r: dict, direction: str, ctx: dict, state: dict) -> str:
    """
    [v6.6.1] Mensagem QUASE — disparada quando token está a <= 5 pts do threshold.

    Mostra todos os pilares do breakdown com pts obtidos, pts máximos e descrição.
    Indica explicitamente quais pilares ainda podem pontuar para fechar o gap.
    Mensagem separada do heartbeat — 1 msg por token elegível, ordenadas por gap.
    """
    score   = r["score"]       if direction == "LONG" else r.get("score_short", 0)
    bd_key  = "breakdown"      if direction == "LONG" else "breakdown_short"
    bd      = r.get(bd_key, [])
    sym     = r.get("base_coin", "?")
    s4h     = r.get("summary_4h", "?")
    s1h     = r.get("summary_1h", "?")
    thr     = ctx["threshold"] if direction == "LONG" else ctx["threshold_short"]
    falta   = thr - score
    trend   = get_score_trend(state, r.get("symbol",""), direction)
    ico     = "🚀" if direction == "LONG" else "📉"
    dq      = r.get("data_quality" if direction == "LONG" else "data_quality_short", 1.0)
    dq_tag  = f"  ⚠️ DQ{dq:.0%}" if dq < 1.0 else ""

    msg  = f"⚠️ <b>QUASE — {direction} {sym}</b>  {trend}  {score}/28  (faltam {falta} pts)\n"
    msg += f"  {s4h} 4H  |  {s1h} 1H  |  thr ≥{thr}{dq_tag}\n"
    msg += f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += f"🧱 <b>PILARES</b>\n"

    fontes_disponiveis = []   # pilares zerados com pts máx > 0 que ainda podem pontuar
    ...
    if bd:
        for pilar, pts, max_pts, detalhe in bd:
            ...
            if len(desc) > 44:
                desc = desc[:42] + "…"
            msg += f"  {ico_p} {pilar:<18} {pts:>+2}/{max_pts}  {desc}\n"
            if pts == 0 and max_pts > 0 and "AUSENTE" not in detalhe:
                fontes_disponiveis.append(f"{pilar.strip()} (+{max_pts})")
    else:
        msg += f"  (breakdown indisponível — análise leve)\n"

    if fontes_disponiveis and falta > 0:
        msg += f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        msg += f"→ Faltam {falta} pts — fontes possíveis:\n"
        msg += "  " + "  |  ".join(fontes_disponiveis[:5]) + "\n"
...
def tg_notify(ctx: dict, results: list, results_short: list,
    ...
    """
    [v6.6.1] Orquestra notificações Telegram ao final de cada scan.

    Fluxo por rodada (ordem de envio):
      1. Heartbeat          — sempre. Contexto de mercado, pipeline, radar,
                              veredicto (AGUARDAR / QUASE / CALLS DISPARADAS).
      2. Msgs QUASE         — 1 por token a <= 5 pts do threshold, ordenadas
                              por gap crescente (mais próximo primeiro).
                              Inclui breakdown completo de pilares.
      3. Msgs de Call       — 1 por token com score >= threshold. Inclui todos
                              os parâmetros operacionais + breakdown de pilares.
    ...
    # QUASE: faltam <= 5 pts, ordenados por gap crescente (mais próximo primeiro)
    quase_long  = sorted(
        [r for r in results       if 0 < thr_l - r["score"]             <= 5],
        key=lambda x: thr_l - x["score"]
    )
    quase_short = sorted(
        [r for r in results_short if 0 < thr_s - r.get("score_short",0) <= 5],
        key=lambda x: thr_s - x.get("score_short", 0)
    )

    total_tent = (1 if TELEGRAM_HEARTBEAT else 0) + len(quase_long) + len(quase_short) + len(alertas_long) + len(alertas_short)
    ...
    # ── 2. Msgs QUASE — 1 por token, mais próximo primeiro ───────────────────
    for r in quase_long:
        msg = _tg_quase(r, "LONG", ctx, state)
        if _tg_send(msg):
            n_enviados += 1
            falta = thr_l - r["score"]
            LOG.info(f"  📲  Telegram QUASE LONG {r['base_coin']} (faltam {falta} pts): enviado ✅")
        else:
            LOG.warning(f"  📵  Telegram QUASE LONG {r['base_coin']}: falha no envio")

    for r in quase_short:
        msg = _tg_quase(r, "SHORT", ctx, state)
        if _tg_send(msg):
            n_enviados += 1
            falta = thr_s - r.get("score_short", 0)
            LOG.info(f"  📲  Telegram QUASE SHORT {r['base_coin']} (faltam {falta} pts): enviado ✅")
        else:
            LOG.warning(f"  📵  Telegram QUASE SHORT {r['base_coin']}: falha no envio")

    # ── 3. Msgs de Call — 1 por token acima do threshold ────────────────────
    for r in alertas_long:
        ...
        if _tg_send(msg):
            ...
            LOG.warning(f"  📵  Telegram CALL LONG {r['base_coin']}: falha no envio")
    ...
    for r in alertas_short:
        ...
        if _tg_send(msg):
            ...
            LOG.warning(f"  📵  Telegram CALL SHORT {r['base_coin']}: falha no envio")
...
def _fetch_okx_tickers_with_oi():
    ...
    try:
        ...
        # Requisição 3: Funding Rates reais [v6.6.1]
...
def _fetch_okx_funding_rates(symbols_okx: list) -> dict:
    """
    [v6.6.1] Busca Funding Rates reais da OKX via endpoint dedicado.
    ...
    headers = {"Accept-Encoding": "gzip, deflate",
               "User-Agent": "Mozilla/5.0 (compatible; scanner/6.6.1)"}
    n_ok = 0; n_err = 0
...
def fetch_perpetuals():
    ...
    LOG.info("📡 [v6.6.1] Iniciando busca de tickers — hierarquia OKX → Gate.io → Bitget")
...
def score_bollinger(d, direction="LONG"):
    ...
    # [v6.6.1] Lógica de descarte assimétrica por direção:
...
def calc_trade_params(price, atr, score=0, threshold=0):
    ...
    Garantia: margem_por_trade ≤ MARGEM_MAX_POR_TRADE em qualquer cenário.
    [v6.6.1] threshold: aviso de margem só é logado se score >= threshold.
    """
...
async def run_scan_async():
    ...
    LOG.info("🚀 Setup Atirador v6.6.1 | Arquitetura 3 Camadas (LONG+SHORT) | Iniciando scan...")
    t_start = time.time()
    ...
    async with aiohttp.ClientSession() as session:
        ...
        # [v6.6.1] Se o batch de candles falhou completamente (retornou {}),
        # tenta requests individuais por coluna para isolar qual nome está quebrando.
        if not tv_15m_candles:
            LOG.warning("  ⚠️  [v6.6.1] Batch candles 15m falhou — tentando colunas individualmente para diagnóstico")
            cols_ok = []
            cols_fail = []
            for col in COLS_15M_CANDLES:
                ...
                if test_result:
                    ...
                    LOG.error(f"  ❌  [v6.6.1] Coluna inválida detectada: {col}")
            if cols_ok:
                LOG.info(f"  🔄  [v6.6.1] Rebuscando candles com {len(cols_ok)} colunas válidas: {cols_ok}")
                tv_15m_candles = await fetch_tv_batch_async(session, symbols_15m, cols_ok)
            if cols_fail:
                LOG.warning(f"  ⚠️  [v6.6.1] Colunas removidas por falha de API: {cols_fail}")
        ...
        report  = f"{'='*58}\n"
        report += f"🎯 SETUP ATIRADOR v6.6.1\n"
        report += f"📅 {ts_full}\n"
        ...
        report += f"💼 GESTÃO DE RISCO — Sizing Risk-First [v6.6.1]\n"
        ...
        LOG.info(report)
        LOG.info(f"✅ Scan v6.6.1 concluído em {elapsed:.1f}s | Fonte: {DATA_SOURCE} | "
