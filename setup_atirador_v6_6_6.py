...
"""
=============================================================================
SETUP ATIRADOR v6.6.6 - Scanner Profissional de Criptomoedas
=============================================================================
...
v6.6.6 (02/04/2026):
  - Observabilidade de Venue (Ações 1 e 2): rastreamento de qual venue
    (Bitget/OKX para klines; BYBIT/BITGET para TradingView) respondeu por
    token no scoring 15m. fetch_klines_async retorna (candles, venue).
    fetch_tv_batch_async retorna (result, tv_venues). venue_info gravado por
    token no RoundLogger e TradeJournal. venue_summary no meta da rodada.
    Aviso discreto ⚠️ em CALLs e QUASEs com quality="mixed".
    Nenhuma lógica de score, threshold ou fallback alterada.

v6.6.5 (01/04/2026):
  - Logging Estruturado (Camada 1): módulo logger.py com RoundLogger.
    Persiste cada rodada em scan_log.jsonl (append-only) + scan_log.db (SQLite/WAL).
    Registra meta (FGI, BTC 4H, thresholds), pipeline (contagens de gates),
    score de cada token com breakdown pilar a pilar, e eventos CALL/QUASE.
  - Forward Testing (Camada 2): módulo journal.py com TradeJournal.
    CALLs (is_hypothetical=0) e QUASEs (is_hypothetical=1) registrados com
    entry, SL, TP1/2/3. check_open_trades() avalia SL/TP via klines 15m antes
    de cada scan. Métricas: win_rate, profit_factor, expectancy por direção.
  - 7 novos comandos Telegram: /log_last, /log_token, /log_quase, /log_calls,
    /perf, /perf_quase, /trade, /log_export.
  - Constante VERSION = "6.6.5" adicionada ao script principal.
...
v6.6.4 (30/03/2026):
  - M01: P1 Bollinger — lógica de exaustão simétrica para LONG e SHORT.
    LONG: de binário (pos<0 → +3) para gradual por proximidade da banda
    inferior (pos<0.15→+3, pos<0.30→+2, pos<0.45→+1).
    SHORT: de continuação (pos<0 → momentum bearish) para exaustão em
    resistência (pos>0.85→+3, pos>0.70→+2, pos>0.55→+1).
    Motivação: simetria conceitual entre LONG e SHORT + evidência empírica
    caso AVAX (30/03/2026) onde sistema identificou resistência corretamente
    mas perdeu timing por ausência de scoring na banda superior.

v6.6.3 (30/03/2026):
  - M03: Multiplicador ATR de SL ajustado de 1.5× para 2.5× em
    calc_trade_params() e calc_trade_params_short(). Piso mínimo de 1.5%
    adicionado (stop_pct = max(stop_pct, 0.015)). Sistema risk-first absorve
    o impacto via redução de alavancagem, sem alterar risco em $.
  - M02: SL ancorado na estrutura detectada pelo sistema. SHORT: SL posicionado
    0.2% acima do OB/resistência mais próxima quando disponível (4H + 1H).
    LONG: 0.2% abaixo do suporte/OB. Helpers _get_nearest_resistance_zone() e
    _get_nearest_support_zone() reutilizam detecção existente sem novos fetches.
    Fallback para ATR quando não há zona estrutural detectada.
  - M04: Gate de confirmação 15m para CALL. Score ≥ threshold com P1 (Bollinger)
    < 2 e P2 (Candles) < 2 simultaneamente é rebaixado para QUASE com aviso
    explícito no log e Telegram. Breakdown inalterado — apenas decisão de emissão
    muda. Flag _m04_quase sinaliza a mensagem QUASE com aviso específico.
  - M14: Watchdog de uptime. Script deploy/watchdog.sh criado. Timestamp gravado
    em /tmp/atirador_last_run.json ao final de cada rodada bem-sucedida.
    Alerta Telegram se scanner offline > 45 minutos. Crontab: */15 * * * *.
...
Autor: Manus AI | v4.1→v6.6.5 (revisão Claude/Anthropic)
=============================================================================
...
# Versão do script — atualizar em todas as ocorrências operacionais a cada sprint
VERSION = "6.6.6"

# [v6.6.5] Módulos de observabilidade — silenciosos na ausência dos arquivos
try:
...
# [v6.6.5] Instâncias globais de runtime — inicializadas em run_scan_async()
_round_logger  = None
_trade_journal = None
...
def _tg_call_long(r: dict, ctx: dict, state: dict = None) -> str:
    ...
    vi = r.get("venue_info", {})
    if vi.get("mixed"):
        msg += f"\n⚠️ Venue mista (klines: {vi['kline_venue']} | TV: {vi['tv_venue']})"
    return msg
...
def _tg_quase(r: dict, direction: str, ctx: dict, state: dict) -> str:
    ...
    m04_aviso = ""
    if r.get("_m04_quase"):
        m04_aviso = "\n⚠️ Score suficiente mas sem confirmação 15m — mantido como QUASE"

    # [v6.6.6] Aviso discreto de venue mista (apenas se mixed == True)
    vi = r.get("venue_info", {})
    venue_aviso = (f"\n⚠️ Venue mista (klines: {vi['kline_venue']} | TV: {vi['tv_venue']})"
                   if vi.get("mixed") else "")
...
def _tg_relatorio_rodada(ctx: dict, total_items: int, qualificados: int,
    ...
    if sem_tv:
        msg += sem_tv
    # [v6.6.6] Resumo de venue dos tokens scoring 15m
    _all_vi = [r.get("venue_info", {}) for r in (results + results_short)]
    if any(_all_vi):
        _n_clean   = sum(1 for v in _all_vi if v.get("quality") == "clean")
        _n_mixed   = sum(1 for v in _all_vi if v.get("quality") == "mixed")
        _n_unknown = sum(1 for v in _all_vi if v.get("quality") == "unknown")
        msg += f"  Venue: {_n_clean} limpos | {_n_mixed} mistos | {_n_unknown} indefinidos\n"
...
def tg_notify(ctx: dict, results: list, results_short: list,
    ...
    # [M04] Gate de confirmação de momentum 15m para CALL
    # Score ≥ threshold com P1 (Bollinger) < 2 E P2 (Candles) < 2 simultaneamente
    # → rebaixa para QUASE com aviso explícito. Breakdown permanece inalterado.
    alertas_long_ok = []
    for r in alertas_long:
        bd = r.get("breakdown", [])
        p1_pts = next((pts for pilar, pts, _, _ in bd if "Bollinger" in pilar), 0)
        p2_pts = next((pts for pilar, pts, _, _ in bd if "Candles"   in pilar), 0)
        if p1_pts < 2 and p2_pts < 2:
            r["_m04_quase"] = True
            LOG.warning(f"  ⚠️ [M04] {r['base_coin']} LONG: score≥thr mas P1={p1_pts} P2={p2_pts} "
                        f"— sem confirmação 15m, rebaixado para QUASE")
            quase_long.append(r)
        else:
            alertas_long_ok.append(r)
    alertas_long = alertas_long_ok
    ...
    for r in alertas_short:
        ...
        if p1_pts < 2 and p2_pts < 2:
            r["_m04_quase"] = True
            LOG.warning(f"  ⚠️ [M04] {r['base_coin']} SHORT: score≥thr mas P1={p1_pts} P2={p2_pts} "
                        f"— sem confirmação 15m, rebaixado para QUASE")
            quase_short.append(r)
        else:
            alertas_short_ok.append(r)
    alertas_short = alertas_short_ok
...
async def fetch_tv_batch_async(session, symbols, columns, retries=3):
    ...
    [v6.6.6]     Retorna (result, tv_venues) onde tv_venues mapeia
                 símbolo → "bybit" | "bitget" | None.
    """
...
def _build_venue_info(kline_venue, tv_venue):
    """
    [v6.6.6] Classifica qualidade de venue por token para observabilidade.

    Regras:
      - Se qualquer venue for None → quality = "unknown", mixed = False
      - kline == tv (mesma venue) → quality = "clean", mixed = False
      - kline = "okx" e tv = "bybit" → quality = "clean", mixed = False
        (OKX e Bybit têm mesma liquidez/microestrutura — mistura aceitável)
      - Qualquer outra combinação diferente → quality = "mixed", mixed = True
        (ex: klines Bitget + TV BYBIT — o caso normal do scanner)

    Não bloqueia nenhum sinal. Apenas classifica para observabilidade.
    """
...
async def fetch_klines_async(session, symbol, granularity="15m", limit=60):
    ...
    [v6.6.6] Retorna (candles, venue) onde venue = "bitget" | "okx" | None.
...
def fetch_perpetuals():
    ...
    LOG.info("📡 [v6.6.3] Iniciando busca de tickers — hierarquia OKX → Gate.io → Bitget")
...
# P1 — Bollinger Bands 15m  (LONG: exaustão em suporte | SHORT: exaustão em resistência)
def score_bollinger(d, direction="LONG"):
    """
    Posição do preço no canal de Bollinger 15m — lógica de exaustão simétrica.
    [v6.6.4 M01] Ambos os lados pontuam por proximidade à banda extrema favorável:
    LONG:  pos < 0.15 → +3 | pos < 0.30 → +2 | pos < 0.45 → +1 (exaustão em suporte)
    SHORT: pos > 0.85 → +3 | pos > 0.70 → +2 | pos > 0.55 → +1 (exaustão em resistência)
    Max: 3 pts.
    """
    price = d.get("price", 0)
    ...
    # [v6.6.4 M01] Lógica de exaustão simétrica:
    #   LONG:  pontuação gradual por proximidade da banda INFERIOR (exaustão em suporte)
    #          pos < 0.15 → extremo inferior → +3 pts
    #          pos < 0.30 → terço inferior   → +2 pts
    #          pos < 0.45 → abaixo do meio   → +1 pt
    #   SHORT: pontuação gradual por proximidade da banda SUPERIOR (exaustão em resistência)
    #          pos > 0.85 → extremo superior → +3 pts
    #          pos > 0.70 → terço superior   → +2 pts
    #          pos > 0.55 → acima do meio    → +1 pt
    #   Simetria: LONG 0.15/0.30/0.45 ↔ SHORT 0.85/0.70/0.55 (espelhos do meio do canal)
    if direction == "SHORT":
        if pos > 1.5:
            LOG.warning(f"    BB {sym}: pos={pos:.0%} fora do range esperado — descartando")
            return 0, f"BB N/A (pos anômala: {pos:.0%})"
        # pos < 0: preço abaixo da banda inferior em contexto SHORT = sem exaustão em resistência
        if pos < 0:
            return 0, f"BB neutro ({pos:.0%}) — abaixo da banda inferior"
        # Exaustão em resistência: pontuação cresce quanto mais próximo da banda superior
        if pos > 0.85:   return 3, f"BB superior ({pos:.0%}) — exaustão em resistência"
        elif pos > 0.70: return 2, f"BB superior ({pos:.0%}) — exaustão em resistência"
        elif pos > 0.55: return 1, f"BB superior ({pos:.0%}) — exaustão em resistência"
        else:            return 0, f"BB neutro ({pos:.0%})"
    else:
        if pos < 0 or pos > 1.5:
            LOG.warning(f"    BB {sym}: pos={pos:.0%} fora do range esperado — descartando")
            return 0, f"BB N/A (pos anômala: {pos:.0%})"
        # Exaustão em suporte: pontuação cresce quanto mais próximo da banda inferior
        if pos < 0.15:   return 3, f"BB inferior ({pos:.0%}) — exaustão em suporte"
        elif pos < 0.30: return 2, f"BB inferior ({pos:.0%}) — exaustão em suporte"
        elif pos < 0.45: return 1, f"BB inferior ({pos:.0%}) — exaustão em suporte"
        else:            return 0, f"BB neutro ({pos:.0%})"
...
# [M02] Helpers — extraem preço da zona estrutural mais próxima
# Reutilizam as funções de detecção existentes sem novos fetches.

def _get_nearest_resistance_zone(candles_4h, candles_1h, price):
    """
    [M02] Retorna o preço da zona de resistência/OB mais próxima ACIMA do preço,
    combinando 4H e 1H. Usado para ancorar SL do SHORT acima da estrutura real.
    Retorna float ou None se não houver zona detectada.
    """
    candidates = []
    # Resistência 4H
    if candles_4h:
        sh, _ = find_swing_points(candles_4h)
        for s in reversed(sh):
            if s["price"] > price:
                candidates.append(s["price"])
                break
        obs_b = detect_order_blocks_bearish(candles_4h)
        for ob in reversed(obs_b[-10:]):
            ob_high = ob["high"]
            if ob_high > price:
    ...
    # Resistência 1H
    if candles_1h:
        sh1, _ = find_swing_points(candles_1h)
        for s in reversed(sh1):
            if s["price"] > price:
                candidates.append(s["price"])
                break
        obs_b1 = detect_order_blocks_bearish(candles_1h)
        for ob in reversed(obs_b1[-10:]):
            ob_high = ob["high"]
            if ob_high > price:
                candidates.append(ob_high)
                break
    if not candidates:
        return None
    # Zona mais próxima acima do preço
    return min(candidates)


def _get_nearest_support_zone(candles_4h, candles_1h, price):
    """
    [M02] Retorna o preço da zona de suporte/OB mais próxima ABAIXO do preço,
    combinando 4H e 1H. Usado para ancorar SL do LONG abaixo da estrutura real.
    Retorna float ou None se não houver zona detectada.
    """
    candidates = []
    # Suporte 4H
    if candles_4h:
        _, sl = find_swing_points(candles_4h)
        for s in reversed(sl):
            if s["price"] < price:
                candidates.append(s["price"])
                break
        obs_b = detect_order_blocks(candles_4h)
        for ob in reversed(obs_b[-10:]):
            ob_low = ob["low"]
            if ob_low < price:
    ...
    # Suporte 1H
    if candles_1h:
        _, sl1 = find_swing_points(candles_1h)
        for s in reversed(sl1):
            if s["price"] < price:
                candidates.append(s["price"])
                break
        obs_b1 = detect_order_blocks(candles_1h)
        for ob in reversed(obs_b1[-10:]):
            ob_low = ob["low"]
            if ob_low < price:
                candidates.append(ob_low)
    ...
    if not candidates:
        return None
    # Zona mais próxima abaixo do preço
    return max(candidates)


def calc_trade_params(price, atr, score=0, threshold=0, zone_price=None):
    """
    [v6.4.0 A1] LONG — sizing risk-first. SL abaixo, TPs acima.

    Fórmula:
      stop_pct  = 2.5 × ATR / preço            [M03] (distância ao SL em %, mín 1.5%)
    ...
    atr_mult = 2.5                           # [M03] 1.5→2.5 — SL mais largo para altcoins voláteis
    stop_pct = (atr_mult * atr) / price      # decimal (ex: 0.05 = 5%)
    stop_pct = max(stop_pct, 0.015)          # piso mínimo 1.5% — M03
    if stop_pct < 0.0005: return None        # SL < 0.05% — inválido para scalp
    sl_atr = price * (1 - stop_pct)

    # [M02] Ancoragem do SL no suporte estrutural detectado (4H/1H)
    # SL final = min(sl_atr, zona_suporte * 0.998) — 0.2% de buffer abaixo do suporte/OB
    # Garante que o SL fique abaixo da zona, evitando ser acionado pelo pullback natural.
    sl = sl_atr
    zone_anchor_log = ""
    if zone_price is not None and zone_price > 0 and zone_price < price:
        sl_zona = zone_price * 0.998   # 0.2% de buffer abaixo do suporte
        if sl_zona < sl_atr:
            sl = sl_zona
            stop_pct = (price - sl) / price
            zone_anchor_log = f" [M02 zona={zone_price:.4f} sl={sl:.4f}]"
            LOG.debug(f"    M02 LONG: SL ancorado em suporte {zone_price:.4f} "
                      f"(ATR={sl_atr:.4f} → zona={sl:.4f})")
...
def calc_trade_params_short(price, atr, score=0, threshold=0, zone_price=None):
    """
    ...
    if not price or not atr or atr <= 0: return None
    atr_mult = 2.5                           # [M03] 1.5→2.5 — SL mais largo para altcoins voláteis
    stop_pct = (atr_mult * atr) / price      # decimal
    stop_pct = max(stop_pct, 0.015)          # piso mínimo 1.5% — M03
    if stop_pct < 0.0005: return None
    sl_atr = price * (1 + stop_pct)          # SL ACIMA (ATR)

    # [M02] Ancoragem do SL na resistência/OB estrutural detectada (4H/1H)
    # SL final = max(sl_atr, zona_resistencia * 1.002) — 0.2% de buffer acima da resistência
    # Garante que o SL fique acima da zona, evitando ser acionado pelo pullback natural.
    sl = sl_atr
    zone_anchor_log = ""
    if zone_price is not None and zone_price > 0 and zone_price > price:
        sl_zona = zone_price * 1.002   # 0.2% de buffer acima da resistência
        if sl_zona > sl_atr:
            sl = sl_zona
            stop_pct = (sl - price) / price
            zone_anchor_log = f" [M02 zona={zone_price:.4f} sl={sl:.4f}]"
            LOG.debug(f"    M02 SHORT: SL ancorado em resistência {zone_price:.4f} "
                      f"(ATR={sl_atr:.4f} → zona={sl:.4f})")
...
# ===========================================================================
# HELPERS DE OBSERVABILIDADE [v6.6.5]
# ===========================================================================

# Mapeamento de nomes de pilar do breakdown para chaves curtas do logger/journal
_PILAR_KEY_MAP = [
    ("P1 Bollinger", "P1"), ("P2 Candles",  "P2"), ("P3 Funding",   "P3"),
    ("P4 Liquidez",  "P4"), ("P5 Figuras",  "P5"), ("P6 CHOCH",     "P6"),
    ("P7",           "P7"), ("P8 Volume",   "P8"), ("P9 OI",        "P9"),
    ("P-1H",         "P1H"),
]


def _breakdown_to_pillars(breakdown: list) -> dict:
    """
    Converte o breakdown de calculate_score() para o formato pillars_dict
    esperado por RoundLogger.add_token() e TradeJournal.open_trade().
    breakdown: [(pilar_name, pts, max_pts, detail), ...]
    Retorna: {"P1": {"score": int, "reason": str}, ...}
    """
    pillars = {}
    for pilar, pts, max_pts, detail in breakdown:
        for prefix, key in _PILAR_KEY_MAP:
            if pilar.startswith(prefix):
                reason = (detail.split("|")[0].strip() if detail else "")
                reason = reason.replace(" ", "_").lower()[:60]
                pillars[key] = {"score": pts, "reason": reason}
                break
    return pillars


def _calcular_sl_tp(d: dict, direction: str) -> tuple:
    """
    Calcula SL/TP hipotéticos para journaling de QUASE.
    Retorna (sl, tp1, tp2, tp3) — tudo 0.0 se não for possível calcular.
    """
    try:
        if direction == "LONG":
            t = calc_trade_params(d["price"], d.get("atr_15m", 0))
        else:
            t = calc_trade_params_short(d["price"], d.get("atr_15m", 0))
        if t:
            return t["sl"], t["tp1"], t["tp2"], t["tp3"]
    except Exception:
        pass
...
async def run_scan_async():
    global LOG, LOG_FILE, TS_SCAN, _round_logger, _trade_journal
    LOG, LOG_FILE, TS_SCAN = setup_logger()

    LOG.info("🚀 Setup Atirador v6.6.5 | Arquitetura 3 Camadas (LONG+SHORT) | Iniciando scan...")
    t_start = time.time()

    # [v6.6.5] Ponto 2 — Inicialização dos módulos de observabilidade
    if _OBSERVABILITY:
        _round_logger  = RoundLogger(version=VERSION)
        _trade_journal = TradeJournal()
        try:
            _trade_journal.check_open_trades()
        except Exception as _e:
            LOG.warning(f"  [obs] check_open_trades falhou: {_e}")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        (tv_4h, _), fg = await asyncio.gather(tv_4h_task, fg_task)
        ...
        tv_1h, _   = await fetch_tv_batch_async(session, symbols_1h, COLS_1H)
        ...
        tv_15m_tech, tv_venues_tech = await fetch_tv_batch_async(session, symbols_15m, COLS_15M_TECH)

        # Request 2: padrões de candle (P2 — se falhar, perde só P2)
        tv_15m_candles, tv_venues_candles = await fetch_tv_batch_async(session, symbols_15m, COLS_15M_CANDLES)
        ...
        if not tv_15m_candles:
            ...
            for col in COLS_15M_CANDLES:
                test_result, _ = await fetch_tv_batch_async(
            ...
            if cols_ok:
                ...
                tv_15m_candles, tv_venues_candles = await fetch_tv_batch_async(session, symbols_15m, cols_ok)
            if cols_fail:
        ...
        # [v6.6.5] Ponto 3 — set_meta: contexto da rodada (candle_lock agora disponível)
        if _round_logger:
            try:
                _round_logger.set_meta(
                    fgi             = fg.get("value", 50),
                    btc_4h          = btc_4h_str,
                    threshold_long  = ctx["threshold"],
                    threshold_short = ctx["threshold_short"],
                    exchange        = DATA_SOURCE,
                    candle_locked   = bool(candle_lock["use_prev"]),
                )
            except Exception as _e:
                LOG.warning(f"  [obs] set_meta falhou: {_e}")

        # [v6.6.5] Ponto 4 — set_pipeline: contagens dos gates
        if _round_logger:
            try:
                _round_logger.set_pipeline(
                    universe      = len(perpetuals),
                    after_gate_4h = len(gate1_passed) + len(gate1_short),
                    after_gate_1h = len(gate2_passed) + len(gate2_short),
                    scored_15m    = len(top_full_long) + len(top_full_short),
                )
            except Exception as _e:
                LOG.warning(f"  [obs] set_pipeline falhou: {_e}")
        ...
        if all_top:
            ...
            # k15m_all[i] is (candles, venue) — unpack into separate maps
            klines_map        = {d["symbol"]: (k15m_all[i][0], k1h_all[i], k4h_all[i])
                                 for i, d in enumerate(all_top)}
            kline_venues_map  = {d["symbol"]: k15m_all[i][1]
                                 for i, d in enumerate(all_top)}
            ...
            for d in top_full_long:
                ...
                if not k15m:
                    ...
                    continue
                # [v6.6.6] Venue observability
                kline_venue_l = kline_venues_map.get(d["symbol"])
                tv_venue_l    = tv_venues_tech.get(d["symbol"]) or tv_venues_candles.get(d["symbol"])
                venue_info_l  = _build_venue_info(kline_venue_l, tv_venue_l)
                d["venue_info"] = venue_info_l
                ...
                # [v6.6.5] Ponto 5 — add_token LONG (antes de qualquer filtragem de OI/trade)
                if _round_logger and sc >= 0:
                    try:
                        _thr_l = ctx["threshold"]
                        _st_l  = ("CALL"  if sc >= _thr_l else
                                  "QUASE" if _thr_l - sc <= QUASE_MARGEM else
                                  "RADAR" if sc >= 5 else "DROP")
                        _round_logger.add_token(
                            symbol=d["symbol"], direction="LONG", score_total=sc,
                            threshold=_thr_l, gap=_thr_l - sc,
                            status=_st_l, pillars=_breakdown_to_pillars(bd),
                            venue_info=venue_info_l,
                        )
                        if _st_l == "QUASE":
                            _round_logger.add_event("QUASE", d["symbol"], "LONG", sc, _thr_l - sc)
                            if _trade_journal:
                                _sl_h, _tp1_h, _tp2_h, _tp3_h = _calcular_sl_tp(d, "LONG")
                                _trade_journal.open_trade(
                                    symbol=d["symbol"], direction="LONG", type="QUASE",
                                    score=sc, entry_price=d["price"],
                                    sl_price=_sl_h, tp1=_tp1_h, tp2=_tp2_h, tp3=_tp3_h,
                                    fgi=fg.get("value", 50), btc_4h=btc_4h_str,
                                    pillars_dict=_breakdown_to_pillars(bd),
                                    kline_venue=venue_info_l.get("kline_venue"),
                                    tv_venue=venue_info_l.get("tv_venue"),
                                    venue_quality=venue_info_l.get("quality"),
                                )
                    except Exception as _e:
                        LOG.warning(f"  [obs] add_token LONG {sym} falhou: {_e}")
                # [v6.3.0 A6] Alerta bloqueado se OI é estimado (não verificado)
                # [M02] Zona de suporte estrutural para ancoragem do SL
                zone_sup = _get_nearest_support_zone(k4h, k1h, d["price"])
                if d.get("oi_estimado"):
                    LOG.warning(f"  ⚠️  {sym}: OI ESTIMADO (fallback vol*0.1) — alerta LONG bloqueado, vai para Observação")
                    d["trade"] = calc_trade_params(d["price"], d.get("atr_15m", 0), score=sc, threshold=ctx["threshold"], zone_price=zone_sup)
                    if d["trade"]: observacoes.append(d)
                    continue
                trade = calc_trade_params(d["price"], d.get("atr_15m", 0), score=sc, threshold=ctx["threshold"], zone_price=zone_sup)
                if trade:
                    ...
                    results.append(d)
                    # [v6.6.6] CALL LONG: registrar evento e abrir trade no journal
                    if _round_logger and sc >= ctx["threshold"]:
                        try:
                            _round_logger.add_event("CALL", d["symbol"], "LONG", sc, 0)
                            if _trade_journal:
                                _trade_journal.open_trade(
                                    symbol=d["symbol"], direction="LONG", type="CALL",
                                    score=sc, entry_price=trade["entry"],
                                    sl_price=trade["sl"], tp1=trade["tp1"],
                            ...
                            LOG.warning(f"  [obs] open_trade CALL LONG {sym} falhou: {_e}")
                else:
            ...
            for d in top_full_short:
                ...
                if not k15m:
                    ...
                    continue
                # [v6.6.6] Venue observability
                kline_venue_s = kline_venues_map.get(d["symbol"])
                tv_venue_s    = tv_venues_tech.get(d["symbol"]) or tv_venues_candles.get(d["symbol"])
                venue_info_s  = _build_venue_info(kline_venue_s, tv_venue_s)
                d["venue_info"] = venue_info_s
                sc, reasons, bd, dq = calculate_score(
                ...
                # [v6.6.6] add_token SHORT (antes de qualquer filtragem de OI/trade)
                if _round_logger and sc >= 0:
                    try:
                        _thr_s = ctx["threshold_short"]
                        _st_s  = ("CALL"  if sc >= _thr_s else
                                  "QUASE" if _thr_s - sc <= QUASE_MARGEM else
                                  "RADAR" if sc >= 5 else "DROP")
                        _round_logger.add_token(
                            symbol=d["symbol"], direction="SHORT", score_total=sc,
                            threshold=_thr_s, gap=_thr_s - sc,
                            status=_st_s, pillars=_breakdown_to_pillars(bd),
                            venue_info=venue_info_s,
                        )
                        if _st_s == "QUASE":
                            _round_logger.add_event("QUASE", d["symbol"], "SHORT", sc, _thr_s - sc)
                            if _trade_journal:
                                _sl_h, _tp1_h, _tp2_h, _tp3_h = _calcular_sl_tp(d, "SHORT")
                                _trade_journal.open_trade(
                                    symbol=d["symbol"], direction="SHORT", type="QUASE",
                                    score=sc, entry_price=d["price"],
                                    sl_price=_sl_h, tp1=_tp1_h, tp2=_tp2_h, tp3=_tp3_h,
                                    fgi=fg.get("value", 50), btc_4h=btc_4h_str,
                                    pillars_dict=_breakdown_to_pillars(bd),
                                    kline_venue=venue_info_s.get("kline_venue"),
                                    tv_venue=venue_info_s.get("tv_venue"),
                                    venue_quality=venue_info_s.get("quality"),
                                )
                    except Exception as _e:
                        LOG.warning(f"  [obs] add_token SHORT {sym} falhou: {_e}")
                # [v6.3.0 A6] Alerta bloqueado se OI é estimado (não verificado)
                # [M02] Zona de resistência estrutural para ancoragem do SL
                zone_res = _get_nearest_resistance_zone(k4h, k1h, d["price"])
                if d.get("oi_estimado"):
                    LOG.warning(f"  ⚠️  {sym}: OI ESTIMADO (fallback vol*0.1) — alerta SHORT bloqueado")
                    continue
                trade = calc_trade_params_short(d["price"], d.get("atr_15m", 0), score=sc, threshold=ctx["threshold_short"], zone_price=zone_res)
                if trade:
                    LOG.info(f"  📉 SHORT {sym}: score={sc}/25 | entry={trade['entry']:.4f} | "
                             f"SL={trade['sl_distance_pct']:.2f}% | alav={trade['alavancagem']}x ✅")
                    d["trade_short"] = trade; d["direction"] = "SHORT"
                    results_short.append(d)
                    # [v6.6.6] CALL SHORT: registrar evento e abrir trade no journal
                    if _round_logger and sc >= ctx["threshold_short"]:
                        try:
                            _round_logger.add_event("CALL", d["symbol"], "SHORT", sc, 0)
                            if _trade_journal:
                                _trade_journal.open_trade(
                                    symbol=d["symbol"], direction="SHORT", type="CALL",
                                    score=sc, entry_price=trade["entry"],
                                    sl_price=trade["sl"], tp1=trade["tp1"],
                                    tp2=trade["tp2"], tp3=trade["tp3"],
                                    fgi=fg.get("value", 50), btc_4h=btc_4h_str,
                            ...
                            LOG.warning(f"  [obs] open_trade CALL SHORT {sym} falhou: {_e}")
                else:
        ...
        report  = f"{'='*58}\n"
        report += f"🎯 SETUP ATIRADOR v6.6.6\n"
        report += f"📅 {ts_full}\n"
        ...
        report += f"💼 SIZING Risk-First [v6.6.3]\n"
        ...
        LOG.info(report)
        LOG.info(f"✅ Scan v6.6.6 concluído em {elapsed:.1f}s | Fonte: {DATA_SOURCE} | Log: {LOG_FILE}")

        # [M14] Watchdog — grava timestamp de execução bem-sucedida
        try:
            import json as _json
            with open("/tmp/atirador_last_run.json", "w") as _f:
                _json.dump({"last_run": datetime.now(BRT).isoformat(), "version": "v6.6.6"}, _f)
...
_ANALISAR_HINTS = {
    ("P1 Bollinger 15m", "LONG") : "→ <45% do canal = +1pt | <30% = +2pts | <15% da banda inferior = +3pts",
    ("P1 Bollinger 15m", "SHORT"): "→ >55% do canal = +1pt | >70% = +2pts | >85% da banda superior = +3pts",
...
async def _analisar_token_inner(symbol: str, t_start: float):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        fg, d, (tv_4h, _) = await asyncio.gather(
        ...
        (tv_1h, _), (tv_tech, _), (tv_candles, _) = await asyncio.gather(
        ...
        (k15m, _), k1h, k4h = await asyncio.gather(
        ...
        # [M02] Zonas estruturais para ancoragem do SL (sem novos fetches)
        zone_sup = _get_nearest_support_zone(k4h, k1h, price)
        zone_res = _get_nearest_resistance_zone(k4h, k1h, price)
        trade_l = calc_trade_params(price, atr_val, score=score_l, threshold=thr_l, zone_price=zone_sup)
        trade_s = calc_trade_params_short(price, atr_val, score=score_s, threshold=thr_s, zone_price=zone_res)
...
def main():
    ...
    if len(sys.argv) >= 3 and sys.argv[1] == "--analisar":
        ...
        # [v6.6.5] Ponto 6 — finally: garantir commit do logger mesmo em exceção
        _t_main = time.time()
        try:
            # Logger e journal inicializados dentro de run_scan_async
            asyncio.run(run_scan_async())
        finally:
            if _round_logger is not None:
                try:
                    if not getattr(_round_logger, "_committed", False):
                        _round_logger.set_exec_seconds(time.time() - _t_main)
                        _round_logger.commit()
                except Exception:
                    pass
