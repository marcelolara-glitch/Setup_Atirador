...
"""
=============================================================================
SETUP ATIRADOR v5.0 - Scanner Profissional de Criptomoedas
=============================================================================
...
v5.0 — Fonte de dados dual (Bybit primário / Bitget fallback) + Universo ampliado:

  [v5.0 — FONTE DUAL]
    Fonte primária: Bybit API pública (perpetuals lineares USDT).
    Fallback automático: Bitget API (comportamento v4.9 preservado).
    Fonte utilizada registrada no LOG e no relatório para rastreabilidade.
    Mapeamento de campos Bybit → estrutura interna idêntica à Bitget.

  [v5.0 — FILTROS RELAXADOS]
    MIN_TURNOVER_24H: $5M → $2M  (captura altcoins com boa liquidez)
    MIN_OI_USD:      $10M → $5M  (OI de $5M ainda garante execução sem slippage)
    TOP_N removido: todos os tokens que passam nos filtros entram no pipeline.
    Gargalo real é KLINE_TOP_N (análise com klines) — não o universo de entrada.

  [v5.0 — LOG DE FONTE]
    Cada execução loga: fonte de dados, total bruto de tickers,
    tokens qualificados e rejeitados por critério.
    Relatório exibe a fonte utilizada na seção de pipeline.

  [v5.0 — FIX LOG cap_score]
    TradeParams: para score < 20 (fora da tabela de alavancagem),
    loga "cap_score=N/A(score<20)" em vez de "cap_score=1x"
    para evitar ambiguidade com cap real pela tabela.

  [v5.0 — FIX "para dobrar banca"]
    Cálculo corrigido: int(BANKROLL / ganho_por_trade) em vez de
    int(BANKROLL/risco/RR)+1 — resultado correto: 10 winners (não 11).
...
Autor: Manus AI | v4.1→v5.0 (revisão Claude/Anthropic)
=============================================================================
"""
...
# ===========================================================================
# DADOS DE MERCADO — Fonte Dual: Bybit (primário) / Bitget (fallback) [v5.0]
# ===========================================================================
# Bybit: GET /v5/market/tickers?category=linear  (sem autenticação)
#   Campos: symbol, turnover24h, openInterest, lastPrice, fundingRate, price24hPcnt
# Bitget: GET /api/v2/mix/market/tickers?productType=USDT-FUTURES (comportamento v4.9)
#   Campos: symbol, usdtVolume, holdingAmount, lastPr, fundingRate, change24h
#
# Ambas retornam perpetuals USDT lineares com volume e OI em tempo real.
# A fonte utilizada é registrada em DATA_SOURCE para log e relatório.
# ===========================================================================

DATA_SOURCE = "desconhecida"   # atualizado por fetch_perpetuals()

def _parse_bybit_tickers(items):
    """
    Normaliza tickers Bybit para a estrutura interna padrão.
    Bybit retorna OI em contratos base; OI em USD = openInterest × lastPrice.
    Bybit retorna price24hPcnt já em decimal (ex: 0.0123 = 1.23%).
    """
    qualified  = []
    ...
    for t in items:
        ...
        turnover = sf(t.get("turnover24h"))
        if turnover < MIN_TURNOVER_24H:
            rej_vol += 1; continue
        price  = sf(t.get("lastPrice"))
        oi_usd = sf(t.get("openInterest")) * price   # OI em contratos × preço = USD
        ...
        qualified.append({
            ...
            "price_change_24h": sf(t.get("price24hPcnt")) * 100,  # decimal → %
        })
...
def _parse_bitget_tickers(items):
    ...
    Bitget retorna OI como holdingAmount (contratos base); OI USD = holding × lastPr.
    Bitget retorna change24h já em decimal (ex: 0.0123 = 1.23%).
    """
    qualified  = []
    rej_symbol = rej_vol = rej_oi = 0
    for t in items:
...
def fetch_perpetuals():
    """
    [v5.0] Busca perpetuals USDT com filtros institucionais.
    Tenta Bybit primeiro; se falhar, usa Bitget como fallback.
    Registra a fonte utilizada em DATA_SOURCE (global) para log e relatório.

    Filtros aplicados (ambas as fontes):
      - Volume 24h (USDT) ≥ MIN_TURNOVER_24H ($2M)
      - Open Interest (USD) ≥ MIN_OI_USD ($5M)
    Resultado ordenado por volume decrescente. Sem limite de quantidade (TOP_N removido).
    """
    global DATA_SOURCE

    # --- Tentativa 1: Bybit ---
    bybit_url = "https://api.bybit.com/v5/market/tickers?category=linear"
    LOG.info("📡 [v5.0] Buscando tickers — Fonte 1: Bybit (perpetuals lineares USDT)...")
    try:
        data  = api_get(bybit_url)
        items = data.get("result", {}).get("list", [])
        if items:
            LOG.debug(f"  Bybit: {len(items)} tickers brutos recebidos")
            qualified, rej_vol, rej_oi = _parse_bybit_tickers(items)
            if qualified:
                qualified.sort(key=lambda x: x["turnover_24h"], reverse=True)
                DATA_SOURCE = "Bybit"
                LOG.info(f"  ✅  [Bybit] {len(qualified)} tokens qualificados | "
                         f"Rejeitados: {rej_vol} vol<${MIN_TURNOVER_24H/1e6:.1f}M, "
                         f"{rej_oi} OI<${MIN_OI_USD/1e6:.0f}M")
                LOG.debug(f"  TOP 5 por volume (Bybit): {[d['base_coin'] for d in qualified[:5]]}")
                return qualified, len(items)
            else:
                LOG.warning("  ⚠️  Bybit retornou tickers mas nenhum passou nos filtros — tentando Bitget")
        else:
            LOG.warning("  ⚠️  Bybit retornou lista vazia — tentando Bitget")
    except Exception as e:
        LOG.warning(f"  ⚠️  Bybit falhou ({type(e).__name__}: {e}) — tentando Bitget")

    # --- Fallback: Bitget ---
    bitget_url = "https://api.bitget.com/api/v2/mix/market/tickers?productType=USDT-FUTURES"
    LOG.info("📡 [v5.0] Fonte 1 indisponível — Fonte 2: Bitget (perpetuals USDT-FUTURES)...")
    data  = api_get(bitget_url)
    items = data.get("data", [])
    LOG.debug(f"  Bitget: {len(items)} tickers brutos recebidos")
    qualified, rej_vol, rej_oi = _parse_bitget_tickers(items)
    qualified.sort(key=lambda x: x["turnover_24h"], reverse=True)
    DATA_SOURCE = "Bitget"
    LOG.info(f"  ✅  [Bitget] {len(qualified)} tokens qualificados | "
             f"Rejeitados: {rej_vol} vol<${MIN_TURNOVER_24H/1e6:.1f}M, "
             f"{rej_oi} OI<${MIN_OI_USD/1e6:.0f}M")
    LOG.debug(f"  TOP 5 por volume (Bitget): {[d['base_coin'] for d in qualified[:5]]}")
    return qualified, len(items)
...
async def run_scan_async():
    ...
    LOG.info("🚀 Setup Atirador v5.0 | Arquitetura 3 Camadas | Iniciando scan...")
    t_start = time.time()
    ...
    async with aiohttp.ClientSession() as session:
        ...
        # [v5.0 FIX] Cálculo correto: BANKROLL / ganho_por_trade = 100 / 10 = 10 winners
        ...
        LOG.info(report)
        LOG.info(f"✅ Scan v5.0 concluído em {elapsed:.1f}s | Fonte: {DATA_SOURCE} | "
