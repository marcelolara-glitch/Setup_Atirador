...
def test_o_registro_tem_a_ficha_e_ela_saiu_do_ar_sem_mover_o_hash():
    # A ficha entrou DESLIGADA no PR que a criou, foi LIGADA num PR de uma linha
    # e voltou a DESLIGAR em 10/09, quando o diag mostrou que a janela dela nao
    # converge a EMA89. Nas tres viradas o hash ficou onde estava — `executar`
    # esta em `FORA_DO_HASH` —, entao a serie de `trades_v10` e uma so; a prova
    # esta em `tests/test_v10_liga_kis_3489.py` e em
    # `tests/test_v10_warmup_420.py`. A sucessora e a `kis_3489_60t_4h_w420`.
