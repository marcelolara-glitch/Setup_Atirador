...
"""
=============================================================================
SETUP ATIRADOR v6.3.0 - Scanner Profissional de Criptomoedas
=============================================================================
...
v6.4.0 (PENDENTE): A1 sizing risk-first, A2 recalibrar score/thresholds/tabela alav,
        A9 separar data_quality de setup_score, A5 venue única (decisão arquitetural)
...
Autor: Manus AI | v4.1→v6.3.0 (revisão Claude/Anthropic)
=============================================================================
...
def _tg_relatorio_rodada(ctx: dict, total_items: int, qualificados: int,
    ...
    if alertas_long or alertas_short:
        ...
        calls_line = f"💤 Sem calls — máx LONG {max_l}/26 | máx SHORT {max_s}/26"
    ...
    for r in results[:7]:
        ...
        radar_long_lines.append(f"  🟢 <b>{sym}</b> {score}/26 {trend} | {s4h}{oi_tag}{fr_tag}")
    ...
    for r in results_short[:7]:
        ...
        fr    = r.get("funding_rate", 0)
        fr_tag = f" FR{fr:.3%}" if fr != 0 else ""
        radar_short_lines.append(f"  🔴 <b>{sym}</b> {score}/26 {trend} | {s4h}{oi_tag}{fr_tag}")
...
def tg_notify(ctx: dict, results: list, results_short: list,
    ...
    """
    [v6.3.0] Orquestra notificações Telegram ao final de cada scan.
...
def fetch_perpetuals():
    """
    [v6.3.0] Busca perpetuals USDT — hierarquia OKX → Gate.io → Bitget.
    ...
    LOG.info("📡 [v6.3.0] Iniciando busca de tickers — hierarquia OKX → Gate.io → Bitget")
...
def calc_trade_params_short(price, atr, score=0):
    """
    [v6.0/v6.1.2] SHORT — SL ACIMA da entrada, TPs ABAIXO.
...
async def run_scan_async():
    ...
    LOG.info("🚀 Setup Atirador v6.3.0 | Arquitetura 3 Camadas (LONG+SHORT) | Iniciando scan...")
    t_start = time.time()
    ...
    async with aiohttp.ClientSession() as session:
        ...
        report  = f"{'='*58}\n"
        report += f"🎯 SETUP ATIRADOR v6.3.0\n"
        report += f"📅 {ts_full}\n"
        ...
        if obs_relevantes:
            ...
            for o in obs_relevantes[:5]:
                ...
                report += f"   · {o['base_coin']} | Score parcial: {o['score']}/26 | {o['summary_4h']} 4H{oi_tag}\n"
        ...
        if obs_short_rel:
            ...
            for o in obs_short_rel[:5]:
                oi_tag = " ⚠️OI?" if o.get("oi_estimado") else ""
                report += f"   · {o['base_coin']} | Score parcial: {o.get('score_short',0)}/26 | {o['summary_4h']} 4H{oi_tag}\n"
        ...
        LOG.info(report)
        LOG.info(f"✅ Scan v6.3.0 concluído em {elapsed:.1f}s | Fonte: {DATA_SOURCE} | "
