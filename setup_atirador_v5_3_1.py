...
"""
=============================================================================
SETUP ATIRADOR v5.3.1.1 - Scanner Profissional de Criptomoedas
=============================================================================
...
v5.3.1 — Reversão da hierarquia de fontes após teste empírico em produção:

  [v5.3.1 — REVERSÃO: OKX volta a ser Fonte 1]
    Teste em produção da v5.3 (22/03/2026 18:03) revelou:
      Gate.io: 651 brutos → apenas 22 qualificados (96.6% rejeitados)
      OKX:     300 brutos → 101 qualificados (66% rejeitados)
    Causa: tokens têm volume real na Binance/Bybit, não na Gate.io.
    Com 22 tokens, o scanner perde cobertura crítica de mercado.

    Problema dos meme coins da OKX (SATS/BONK/NEIRO no top) é COSMÉTICO:
    os gates técnicos (4H/1H) eliminam esses tokens imediatamente por
    ausência de dados no TradingView ou sinal SELL/STRONG_SELL.
    Eles entram no universo mas nunca chegam à análise técnica.

    Gate.io mantida como Fonte 2 (fallback melhor que Bitget para tokens
    OKX-exclusivos que precisam de klines via OKX de qualquer forma).

    Hierarquia final definitiva: OKX → Gate.io → Bitget

  [NOTA TÉCNICA — LIMITAÇÃO RECONHECIDA]
    A solução ideal para a Etapa 1 (coleta de tickers) seria a API da
    CoinGlass (https://coinglass.com/pricing), que retorna Open Interest
    AGREGADO de 30+ exchanges simultâneas (Binance + Bybit + OKX + Bitget
    + Gate.io etc.) em uma única chamada, eliminando o viés de dados locais
    de qualquer exchange individual.

    Por que não foi implementada: CoinGlass não possui plano gratuito com
    acesso à API. Os planos começam em ~$35/mês (Hobby) e oferecem endpoints
    como /api/futures/coins-markets com volume, OI agregado e funding rate
    de todo o mercado em uma chamada. Para uso em produção contínua, é a
    solução mais robusta e tecnicamente correta disponível.

    Alternativas gratuitas investigadas e descartadas (22/03/2026):
      Binance fapi:  geo-block Brasil confirmado (HTTP 403)
      Bybit:         geo-block Brasil confirmado (HTTP 403 / timeout 92s)
      CoinGecko:     endpoint /derivatives genérico inutilizável (21k itens
                     de exchanges mistas sem normalização); endpoint específico
                     /derivatives/exchanges/{id}/tickers retorna HTTP 404
                     (IDs incorretos + possivelmente requer plano pago)
      Laevitas:      Enterprise — sem acesso público
      The Block:     Sem API operacional adequada para este uso
      Velo Data:     Sem plano gratuito documentado
      Coinalyze:     Máx 20 símbolos por chamada (inviável para 300+ tokens)
      CoinGlass free: Requer cadastro mas endpoints úteis (OI, markets) são pagos

    Conclusão: não existe fonte gratuita de dados AGREGADOS de futuros
    acessível de IPs brasileiros. A OKX é a melhor opção gratuita disponível,
    aceitando o trade-off de dados locais sobre dados globais agregados.

v5.3: Gate.io como Fonte 1 (revertido — universo insuficiente em produção)
...
Autor: Manus AI | v4.1→v5.3.1 (revisão Claude/Anthropic)
=============================================================================
"""
...
def fetch_perpetuals():
    """
    [v5.3.1] Busca perpetuals USDT — hierarquia OKX → Gate.io → Bitget.
    ...
    LOG.info("📡 [v5.3.1] Iniciando busca de tickers — hierarquia OKX → Gate.io → Bitget")
...
async def run_scan_async():
    ...
    LOG.info("🚀 Setup Atirador v5.3.1 | Arquitetura 3 Camadas | Iniciando scan...")
    t_start = time.time()
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if not gate2_passed:
            ...
            report  = f"{'='*55}\n"
            report += f"🎯 SETUP ATIRADOR v5.3.1\n"
            report += f"📅 {ts_full}\n"
        ...
        LOG.info(report)
        LOG.info(f"✅ Scan v5.3.1 concluído em {elapsed:.1f}s | Fonte: {DATA_SOURCE} | "
