...
# sobre a excursao 15m, usando o caminho fav/adv por barra exposto por
# measure_event. Ambiguidade residual existe APENAS quando alvo e stop caem
# na MESMA barra 15m -> resolvida pessimisticamente (-S) e reportada AMB_SB.
...
def run(store: str, symbols: list, start_iso: str, end_iso: str) -> dict:
    """Por simbolo: timeline de regime 15m -> entradas TREND-aligned ->
    measure_event -> first_touch em cada celula S x T x H. Acumula retornos
    BRUTOS (fracao do nocional); custos entram apenas no main()."""
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Estagio 0.5 — primeiro toque exato sobre excursao 15m")
    ...
    r = run(args.store, symbols, args.start, args.end)
    if r["total"] == 0:
        print("SEM ENTRADAS — veredito nao emitido")
        return 1

    print("\n===== BRACKET ESTAGIO 0.5 (primeiro toque exato, 15m) =====")
